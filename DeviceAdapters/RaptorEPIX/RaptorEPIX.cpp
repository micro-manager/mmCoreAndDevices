...
#else
	const char* g_strVersion = "v1.16.3, 5/11/2022";
#endif
...
int CRaptorEPIX::SetSystemState(unsigned char nState) const
{
	...
	{
		return serialWriteReadCmd(UNITSOPENMAP, UNITMASK, bufin,  3, buf, 256, 2, false );
	}	
}
...
//	double dCurrentClock1 = myClock();
   ...
   //md.PutImageTag("FieldCount", CDeviceUtils::ConvertToString( (long) fieldCount_)); 
	...
	md.PutImageTag("Interval Wait Time", CDeviceUtils::ConvertToString(myIntervalWaitTime_*1000.0));

	md.PutImageTag("Frame Diff Time", CDeviceUtils::ConvertToString(myFrameDiffTime_*1000.0));
	...
	if(trigSnap_)
	{
		md.PutImageTag("Trigger to Capture Time",
			CDeviceUtils::ConvertToString((myCaptureTime_ - myReadoutStartTime_)*1000.0));

		md.PutImageTag("Trigger to Data Time",
			CDeviceUtils::ConvertToString((myCaptureTime2_ - myReadoutStartTime_)*1000.0));
	}
	md.PutImageTag("Field Count", CDeviceUtils::ConvertToString(fieldCount_));

	md.PutImageTag("Field Buffer", CDeviceUtils::ConvertToString(fieldBuffer_));
...
int CRaptorEPIX::OnCCDTemp(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   if (eAct == MM::BeforeGet)
   {
		if(!IsCapturing()) //(nCCDTempCount_>10)
		{
		  ...
		  if(ForceUpdate_ || (ExtTrigStatus_ & 0x04))
		  {
 	  	  GetSystemState();
...
*/
    ...
    for (int u = 0; u < 1; u++) 
	{
		...
		if(triggerMode_<=1 || (_IS_CAMERA_OWL_FAMILY))
		{
			pxd_goAbortLive(UNITMASK); 
			captureMode_ = 0;
