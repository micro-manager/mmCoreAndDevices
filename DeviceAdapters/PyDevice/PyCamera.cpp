...
using namespace std;
const double CPyCamera::nominalPixelSizeUm_ = 1.0;
...
///////////////////////////////////////////////////////////////////////////////
// CPyCamera implementation
// ~~~~~~~~~~~~~~~~~~~~~~~~~~

/**
* CPyCamera constructor.
    ...
    imgManpl_(0)
{
    ...
    CreateStringProperty("DAC port analog out x", dacportoutx_.c_str(), false,
        new CPropertyAction(this, &CPyCamera::OnDACPortOutx),
        true);

    CreateStringProperty("DAC port analog out y", dacportouty_.c_str(), false,
        new CPropertyAction(this, &CPyCamera::OnDACPortOuty),
        true);

    CreateStringProperty("DAC port analog in", dacportin_.c_str(), false,
        new CPropertyAction(this, &CPyCamera::OnDACPortIn),
        true);*/
}

/**
* CPyCamera destructor.
...
int CPyCamera::Initialize()
{
    ...
    // binning
    CPropertyAction* pAct = new CPropertyAction(this, &CPyCamera::OnBinning);
    ...
    // pixel type
    pAct = new CPropertyAction(this, &CPyCamera::OnPixelType);
    ...
    // Bit depth
    pAct = new CPropertyAction(this, &CPyCamera::OnBitDepth);
    ...
    pAct = new CPropertyAction(this, &CPyCamera::OnZoomFactor);
    CreateFloatProperty("ZoomFactor", 1.0, false, pAct);
    
    pAct = new CPropertyAction(this, &CPyCamera::OnDelay);
    CreateIntegerProperty("Delay", 0, false, pAct);

    pAct = new CPropertyAction(this, &CPyCamera::OnDwelltime);
    CreateFloatProperty("Dwelltime", 4, false, pAct);

    pAct = new CPropertyAction(this, &CPyCamera::OnScanpadding);
    CreateFloatProperty("Scanpadding", 1.0, false, pAct);

     // scan mode
    pAct = new CPropertyAction(this, &CPyCamera::OnScanMode);
    ...
    pAct = new CPropertyAction(this, &CPyCamera::OnScanXSteps);
    CreateIntegerProperty("ScanXSteps", 512, false, pAct);
    pAct = new CPropertyAction(this, &CPyCamera::OnScanYSteps);
    CreateIntegerProperty("ScanYSteps", 512, false, pAct);

    pAct = new CPropertyAction(this, &CPyCamera::OnInputMin);
    CreateFloatProperty("InputMin", -2.0, false, pAct);
    pAct = new CPropertyAction(this, &CPyCamera::OnInputMax);
    ...
    // Trigger device
    pAct = new CPropertyAction(this, &CPyCamera::OnTriggerDevice);
    CreateStringProperty("TriggerDevice", "", false, pAct);

    // Camera mode: 
    pAct = new CPropertyAction(this, &CPyCamera::OnMode);
 ...
 */
double CPyCamera::GetSequenceExposure()
{
...
int CPyCamera::GetExposureSequenceMaxLength(long& nrEvents) const
{
    if (!isSequenceable_) {
...
int CPyCamera::StartExposureSequence()
{
    if (!isSequenceable_) {
        return DEVICE_UNSUPPORTED_COMMAND;
    }
...
int CPyCamera::StopExposureSequence()
{
    if (!isSequenceable_) {
        return DEVICE_UNSUPPORTED_COMMAND;
    }
 ...
 */
int CPyCamera::ClearExposureSequence()
{
    if (!isSequenceable_) {
        return DEVICE_UNSUPPORTED_COMMAND;
    }
 ...
 */
int CPyCamera::AddToExposureSequence(double exposureTime_ms)
{
    if (!isSequenceable_) {
        return DEVICE_UNSUPPORTED_COMMAND;
    }
...
int CPyCamera::SendExposureSequence() const {
    if (!isSequenceable_) {
        return DEVICE_UNSUPPORTED_COMMAND;
    }
...
int CPyCamera::SetAllowedBinning()
{ 
 ...
 */
int CPyCamera::RunSequenceOnThread()
{
    int ret = DEVICE_ERR;
    MM::MMTime startTime = GetCurrentMMTime();
 ...
 */
void CPyCamera::OnThreadExiting() throw()
{
    try
    {
...
MySequenceThread::MySequenceThread(CPyCamera* pCam)
...
*/
int CPyCamera::OnBinning(MM::PropertyBase* pProp, MM::ActionType eAct)
{
    int ret = DEVICE_ERR;
    switch (eAct)
    {
...
*/
int CPyCamera::OnPixelType(MM::PropertyBase* pProp, MM::ActionType eAct)
{
    int ret = DEVICE_ERR;
    switch (eAct)
    {
...
*/
int CPyCamera::OnBitDepth(MM::PropertyBase* pProp, MM::ActionType eAct)
{
    int ret = DEVICE_ERR;
    switch (eAct)
    {
...
*/
int CPyCamera::OnScanMode(MM::PropertyBase* pProp, MM::ActionType eAct)
{
    if (eAct == MM::AfterSet) {
        pProp->Get(scanMode_);
        SetAllowedBinning();
...
int CPyCamera::OnScanXSteps(MM::PropertyBase* pProp, MM::ActionType eAct)
{
...
int CPyCamera::OnZoomFactor(MM::PropertyBase* pProp, MM::ActionType eAct)
{
    if (eAct == MM::BeforeGet)
    {
...
int CPyCamera::OnDelay(MM::PropertyBase* pProp, MM::ActionType eAct)
{
    if (eAct == MM::BeforeGet)
    {
        pProp->Set(delay_);
...
int CPyCamera::OnDwelltime(MM::PropertyBase* pProp, MM::ActionType eAct)
{
    if (eAct == MM::BeforeGet)
    {
...
int CPyCamera::OnScanpadding(MM::PropertyBase* pProp, MM::ActionType eAct)
{
    if (eAct == MM::BeforeGet)
    {
...
int CPyCamera::OnInputMin(MM::PropertyBase* pProp, MM::ActionType eAct)
{
    if (eAct == MM::BeforeGet)
    {
        pProp->Set(inputmin_);
...
int CPyCamera::OnInputMax(MM::PropertyBase* pProp, MM::ActionType eAct)
{
    if (eAct == MM::BeforeGet)
    {
        pProp->Set(inputmax_);
...
int CPyCamera::OnScanYSteps(MM::PropertyBase* pProp, MM::ActionType eAct)
{
    if (eAct == MM::BeforeGet)
    {
...
int CPyCamera::OnTriggerDevice(MM::PropertyBase* pProp, MM::ActionType eAct)
{
    if (eAct == MM::BeforeGet)
    {
...
int CPyCamera::OnDACPortOutx(MM::PropertyBase* pProp, MM::ActionType eAct)
{
    if (eAct == MM::BeforeGet)
    {
...
int CPyCamera::OnDACPortOuty(MM::PropertyBase* pProp, MM::ActionType eAct)
{
    if (eAct == MM::BeforeGet)
    {
...
int CPyCamera::OnDACPortIn(MM::PropertyBase* pProp, MM::ActionType eAct)
{
    if (eAct == MM::BeforeGet)
    {
...
int CPyCamera::OnIsSequenceable(MM::PropertyBase* pProp, MM::ActionType eAct)
{
    std::string val = "Yes";
    if (eAct == MM::BeforeGet)
    {
...
int CPyCamera::OnMode(MM::PropertyBase* pProp, MM::ActionType eAct)
{
    std::string val;
    if (eAct == MM::BeforeGet)
    {
...
*/
int CPyCamera::ResizeImageBuffer()
{
    char buf[MM::MaxStrLength];
    //int ret = GetProperty(MM::g_Keyword_Binning, buf);
    //if (ret != DEVICE_OK)
...
void CPyCamera::GenerateEmptyImage(ImgBuffer& img)
{
    MMThreadGuard g(imgPixelsLock_);
