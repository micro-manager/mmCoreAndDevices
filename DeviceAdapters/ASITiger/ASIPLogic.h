...
private:
    ...
    unsigned int numCells_ = 16; // will read actual number of positions later
    unsigned int currentPosition_ = 1; // cached value of current position
    ...
    bool useAsdiSPIMShutter_ = false;
    bool useAs4ChShutter_ = false;
    bool useAs7ChShutter_ = false;
    // used together with either useAs4ChShutter_ or useAs7ChShutter_,
    // takes into account address 41 backplane (TTL0 for CameraA)

    bool shutterOpen_ = false;
    bool editCellUpdates_ = true;
    bool advancedPropsEnabled_ = false; // flag to only create advanced properties once
};
