...
ReflectionFocus::ReflectionFocus() :
   ...
   stopThread_(false),
   status_("Off"),
   imageAnalyzer_(new DetectTwoBrightSpots())
{
   ...
   // Name
   CreateProperty(MM::g_Keyword_Name, "ReflectionFocus", MM::String, true);
   // Description
...
void ReflectionFocus::GetName(char* name) const
{
   CDeviceUtils::CopyLimitedString(name, "ReflectionFocus");
}

int ReflectionFocus::Initialize()
{
   ...
   CPropertyAction* pAct = new CPropertyAction(this, &ReflectionFocus::OnShutter);
   ...
   pAct = new CPropertyAction(this, &ReflectionFocus::OnCamera);
   ...
   pAct = new CPropertyAction(this, &ReflectionFocus::OnFocusStage);
   ...
   pAct = new CPropertyAction(this, &ReflectionFocus::OnAlgorithm);
   CreateProperty(g_Alg, imageAnalyzer_->GetName(), MM::String, false, pAct);
   AddAllowedValue(g_Alg, DetectTwoBrightSpots::GetAnalyzerName());


   // Create ROI-X property
   pAct = new CPropertyAction(this, &ReflectionFocus::OnROI_X);
   ...
   pAct = new CPropertyAction(this, &ReflectionFocus::OnExposure);
   ...
   pAct = new CPropertyAction(this, &ReflectionFocus::OnSpotSelection);
   ...
   pAct = new CPropertyAction(this, &ReflectionFocus::OnMeasureOffset);
   ...
   stopThread_ = false;
   continuousFocusLocked_ = false;
   continuousFocusThread_ = std::thread(&ReflectionFocus::ContinuousFocusThread, this);
...
int ReflectionFocus::OnShutter(MM::PropertyBase* pProp, MM::ActionType eAct)
{
...
int ReflectionFocus::OnCamera(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   if (eAct == MM::BeforeGet) {
...
int ReflectionFocus::OnFocusStage(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   if (eAct == MM::BeforeGet) {
...
int ReflectionFocus::OnAlgorithm(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   if (eAct == MM::BeforeGet) {
      pProp->Set(imageAnalyzer_->GetName());
   }
   else if (eAct == MM::AfterSet) {
      std::string algName;
      pProp->Get(algName);
      if (algName == DetectTwoBrightSpots::GetAnalyzerName())
      {
         delete imageAnalyzer_;
         imageAnalyzer_ = new DetectTwoBrightSpots();
      }
      else
      {
         // Unknown algorithm - should not happen due to allowed values
...
int ReflectionFocus::OnROI_X(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   if (eAct == MM::BeforeGet)
   {
...
int ReflectionFocus::OnROI_Y(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   if (eAct == MM::BeforeGet)
   {
...
int ReflectionFocus::OnROI_Width(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   if (eAct == MM::BeforeGet)
   {
...
int ReflectionFocus::OnROI_Height(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   if (eAct == MM::BeforeGet)
   {
...
int ReflectionFocus::OnBinning(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   if (eAct == MM::BeforeGet)
   {
      pProp->Set(binning_);
...
int ReflectionFocus::OnExposure(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   if (eAct == MM::BeforeGet)
   {
...
int ReflectionFocus::OnDeviceSettings(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   if (eAct == MM::BeforeGet)
   {
...
int ReflectionFocus::OnSpotSelection(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   if (eAct == MM::BeforeGet)
   {
...
int ReflectionFocus::OnPrecision(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   if (eAct == MM::BeforeGet)
   {
...
int ReflectionFocus::OnMaxZ(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   if (eAct == MM::BeforeGet)
   {
      pProp->Set(maxZ_);
...
int ReflectionFocus::OnStatus(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   if (eAct == MM::BeforeGet)
   {
...
int ReflectionFocus::OnDeviceSettingsDescription(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   if (eAct == MM::BeforeGet)
   {
...
int ReflectionFocus::OnCalibrate(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   if (eAct == MM::BeforeGet)
   {
      pProp->Set("Idle");
...
int ReflectionFocus::OnMeasureOffset(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   if (eAct == MM::BeforeGet)
   {
      pProp->Set("Idle");
...
bool ReflectionFocus::Busy()
{
   return false;
}

int ReflectionFocus::SetContinuousFocusing(bool on)
{
   {
...
int ReflectionFocus::GetContinuousFocusing(bool& on)
{
   on = continuousFocusing_;
   return DEVICE_OK;
}

bool ReflectionFocus::IsContinuousFocusLocked()
{
   return continuousFocusLocked_;
}

int ReflectionFocus::SetOffset(double offset)
{
   offset_ = offset;
...
int ReflectionFocus::GetOffset(double& offset)
{
   offset = offset_;
   return DEVICE_OK;
}

void ReflectionFocus::RegisterStage(MM::Stage* stage)
{
   if (stage == nullptr)
      return;
...
void ReflectionFocus::UnregisterStage(MM::Stage* stage)
{
   if (stage == nullptr)
      return;
...
void ReflectionFocus::NotifyRegisteredStages()
{
   std::lock_guard<std::mutex> lock(registrationMutex_);
...
int ReflectionFocus::FullFocus()
{
   ...
   while (iteration < maxIterations)
   {
      ...
      if (targetZ > maxZ_)
         return ERR_TARGET_TOO_HIGH;  // Would exceed maximum Z limit
...
int ReflectionFocus::IncrementalFocus()
{
   return SnapAndAnalyze();
}

void ReflectionFocus::ContinuousFocusThread()
{
   while (!stopThread_)
   {
...
void ReflectionFocus::UpdateStatus(const std::string& newStatus)
{
...
int ReflectionFocus::GetLastFocusScore(double& score)
{
   score = lastSpotScore_;
   return DEVICE_OK;
}

int ReflectionFocus::GetCurrentFocusScore(double& score)
{
   // is this current?
   score = lastSpotScore_;
   return DEVICE_OK;
...
int ReflectionFocus::SnapAndAnalyze()
{
   // Get shutter device
   ...
   double score1, x1, y1;
   double score2, x2, y2;
   imageAnalyzer_->AnalyzeImage(resultImage, score1, x1, y1, score2, x2, y2);
...
int ReflectionFocus::GetImageFromBuffer(ImgBuffer& img)
{
   // Get camera device
   MM::Camera* camera = static_cast<MM::Camera*>(GetDevice(camera_.c_str()));
   if (camera == nullptr)
...
int ReflectionFocus::SetCameraBinning()
{
   if (camera_ == "" || camera_ == "Undefined")
   {
...
int ReflectionFocus::SetCameraROI()
{
   if (camera_ == "" || camera_ == "Undefined")
   {
      // even though we are not setting binning, things get complicated if we return an error here
...
int ReflectionFocus::PerformCalibration()
{
   ...
   ret = imageAnalyzer_->AnalyzeImage(resultImage, score1, x1, y1, score2, x2, y2);
   ...
   for (int i = 0; i < numSteps; i++)
   {
      ...
      // Analyze to find spot position
      ret = imageAnalyzer_->AnalyzeImage(resultImageLoop, score1, x1, y1, score2, x2, y2);
      if (ret != DEVICE_OK)
      {
         pStage->SetPositionUm(originalPos);
         return ret;
...
int ReflectionFocus::PerformMeasureOffset()
{
...
int ReflectionFocus::ValidateZPosition(double targetZ)
{
   if (focusStage_ == "" || focusStage_ == "Undefined")
      return ERR_NO_PHYSICAL_STAGE;
...
std::string ReflectionFocus::GetCalibrationFilePath()
{
#ifdef _WIN32
...
int ReflectionFocus::SaveCalibrationData()
{
...
int ReflectionFocus::LoadCalibrationData()
{
   std::string filePath = GetCalibrationFilePath();
