...
private:
	...
	void AddAllowedDivisionPropXY(const char* div);   // Added 5月12日2022   t.abed
	void CreateChanelProp();
	int SetDivision(int division);
	int SetSpeed(int val);
	int UpdatePositionZ();    // Added 4月7日2022　　　t.abed
	int DriveCommadProcessZ(double position);// Added 4月7日2022   t.abed
	...
	std::string channel_;
	int speed_;
	double stepsZ_; // Z stage current position Added 4月7日2022   t.abed
