...
inline auto GetModInfoSerialNo(ModInfo const &mod_info) noexcept -> std::string {
	...
	std::copy(mod_info.serial_no, mod_info.serial_no + serial_no_siz, serial_no.begin());
...
struct DCCModInfoWrap : DCCModInfo {
	...
	auto SerialNo() const noexcept -> std::string { return GetModInfoSerialNo(*this); }
};
...
inline auto DCCDCUGetErrorString(short err) noexcept -> std::string {
	...
	auto e = DCC_get_error_string(err, buf.data(), static_cast<short>(buf.size()));
	if (e) {
...
template <>
class DCCDCUConfig<DCCOrDCU::DCC> {
public:
	...
	static auto GetInitStatus(short mod_no, short *ini_status) noexcept -> short {
		return DCC_get_init_status(mod_no, ini_status);
	}

	static auto GetMode() noexcept -> short {
		return DCC_get_mode();
	}

	static auto GetModuleInfo(short mod_no, ModInfoType *mod_info) noexcept -> short {
		return DCC_get_module_info(mod_no, mod_info);
	}

	static auto GetParameter(short mod_no, short par_id, float *value) noexcept -> short {
		return DCC_get_parameter(mod_no, par_id, value);
	}

	static auto SetParameter(short mod_no, short par_id, short send_to_hard, float value) noexcept -> short {
		return DCC_set_parameter(mod_no, par_id, send_to_hard, value);
	}

	static auto GetGainHVLimit(short mod_no, short con_no, short *value) noexcept -> short {
	...
	static auto Close() noexcept -> short {
		return DCC_close();
	}

	static auto ConnectorHasFeature(short con_no, ConnectorFeature feature) noexcept -> bool {
		switch (con_no) {
		case 0:
	...
	static auto ConnectorParameterId(short con_no, ConnectorFeature feature) noexcept -> short {
		auto const p = std::make_pair(static_cast<int>(con_no), feature);

		if (p == std::make_pair(0, ConnectorFeature::Plus5V)) return C1_P5V;
		if (p == std::make_pair(0, ConnectorFeature::Minus5V)) return C1_M5V;
		if (p == std::make_pair(0, ConnectorFeature::Plus12V)) return C1_P12V;
		if (p == std::make_pair(0, ConnectorFeature::GainHV)) return C1_GAIN_HV;

		if (p == std::make_pair(1, ConnectorFeature::Plus5V)) return C2_P5V;
		if (p == std::make_pair(1, ConnectorFeature::Minus5V)) return C2_M5V;
		if (p == std::make_pair(1, ConnectorFeature::Plus12V)) return C2_P12V;
		if (p == std::make_pair(1, ConnectorFeature::DigitalOut)) return C2_DIGOUT;

		if (p == std::make_pair(2, ConnectorFeature::Plus5V)) return C3_P5V;
		if (p == std::make_pair(2, ConnectorFeature::Minus5V)) return C3_M5V;
		if (p == std::make_pair(2, ConnectorFeature::Plus12V)) return C3_P12V;
		if (p == std::make_pair(2, ConnectorFeature::GainHV)) return C3_GAIN_HV;
		if (p == std::make_pair(2, ConnectorFeature::Cooling)) return C3_COOLING;
		if (p == std::make_pair(2, ConnectorFeature::CoolerVoltage)) return C3_COOLVOLT;
...
template <>
class DCCDCUConfig<DCCOrDCU::DCU> {
public:
	...
	static auto ConnectorParameterId(short con_no, ConnectorFeature feature) noexcept -> short {
		...
		if (p == std::make_pair(0, ConnectorFeature::Plus5V)) return C1U_P5V;
		if (p == std::make_pair(0, ConnectorFeature::Minus5V)) return C1U_M5V;
		if (p == std::make_pair(0, ConnectorFeature::Plus12V)) return C1U_P12V;
		if (p == std::make_pair(0, ConnectorFeature::GainHV)) return C1U_GAIN_HV;
		if (p == std::make_pair(0, ConnectorFeature::Cooling)) return C1U_COOLING;
		if (p == std::make_pair(0, ConnectorFeature::CoolerVoltage)) return C1U_COOLVOLT;
		if (p == std::make_pair(0, ConnectorFeature::CoolerCurrentLimit)) return C1U_COOLCURR;

		if (p == std::make_pair(1, ConnectorFeature::Plus5V)) return C2U_P5V;
		if (p == std::make_pair(1, ConnectorFeature::Minus5V)) return C2U_M5V;
		if (p == std::make_pair(1, ConnectorFeature::Plus12V)) return C2U_P12V;
		if (p == std::make_pair(1, ConnectorFeature::GainHV)) return C2U_GAIN_HV;
		if (p == std::make_pair(1, ConnectorFeature::Cooling)) return C2U_COOLING;
		if (p == std::make_pair(1, ConnectorFeature::CoolerVoltage)) return C2U_COOLVOLT;
		if (p == std::make_pair(1, ConnectorFeature::CoolerCurrentLimit)) return C2U_COOLCURR;

		if (p == std::make_pair(2, ConnectorFeature::Plus5V)) return C3U_P5V;
		if (p == std::make_pair(2, ConnectorFeature::Minus5V)) return C3U_M5V;
		if (p == std::make_pair(2, ConnectorFeature::Plus12V)) return C3U_P12V;
		if (p == std::make_pair(2, ConnectorFeature::GainHV)) return C3U_GAIN_HV;
		if (p == std::make_pair(2, ConnectorFeature::Cooling)) return C3U_COOLING;
		if (p == std::make_pair(2, ConnectorFeature::CoolerVoltage)) return C3U_COOLVOLT;
		if (p == std::make_pair(2, ConnectorFeature::CoolerCurrentLimit)) return C3U_COOLCURR;

		if (p == std::make_pair(3, ConnectorFeature::Plus5V)) return C4U_P5V;
		if (p == std::make_pair(3, ConnectorFeature::Minus5V)) return C4U_M5V;
		if (p == std::make_pair(3, ConnectorFeature::Plus12V)) return C4U_P12V;
		if (p == std::make_pair(3, ConnectorFeature::GainHV)) return C4U_GAIN_HV;
		if (p == std::make_pair(3, ConnectorFeature::Cooling)) return C4U_COOLING;
		if (p == std::make_pair(3, ConnectorFeature::CoolerVoltage)) return C4U_COOLVOLT;
		if (p == std::make_pair(3, ConnectorFeature::CoolerCurrentLimit)) return C4U_COOLCURR;
