...
= > We confirm connection with "kyp_lcd_screen t=6s \"\n  Micromanager\n  connected.\"", crlf
    Once connected...we set the xyz stage control to "voltage mode" = 2  
    and get the XYZ stage ranges
= > if no response or range = 0..MM thinks there's no stage
= > we set the stuv power board DAC lines to "current mode" = 3
...
;
    ...
    LogMessage("Sent splash Screen and didn't wait for 5 seconds!!", false);

    ret = GetSerialAnswer(port_.c_str(), "W>", answer);
      if ( ret != DEVICE_OK ) return ret;
 
    //completed initialisation okay...
    version = 99;
...
// I am assuming that when this function (method) is called the passed parameter "value"
...
int CWOSMSwitch::WriteToPort(long value)
{
    ...
    // 16 15 14 13 12 11 10 09 08 07 06 05 04 02 01 :: 16 15 14 13 12 11 10 09 08 07 06 05 04 02 01
    ...
    // e.g. a starting value of "90" would have the bit pattern
    ...
    leng = snprintf(( char* ) command, 50, "dig_out %d\r\n", value);
...
int CWOSMShutter::WriteToPort(long value)
{
    ...
    // iterate through shutter control lines just do line 'w' = ('s' + 4) 
    for ( int i = 4; i < 5; i++ ) {
        chan[0] = 's' + i; chan[1] = 0;           // iterate through s, t, u, v, w lines on WOSM

        leng = snprintf(( char* ) command, 50, "dig_out %s %d\r\n", (char*) chan, value);
        ret = hub->WriteToComPortH(( const unsigned char* ) command, leng);
        if ( ret != DEVICE_OK )  return ret;
...
int CWOSMShutter::OnOnOff(MM::PropertyBase* pProp, MM::ActionType eAct)
{
    ...
    else if ( eAct == MM::AfterSet )
    {
        ...
        if ( pos == 0 )
            ...
            ret = WriteToPort(1); //  Shutter open (and/or restore LED O/P pattern) <- I don't want "Shutter" to mess with switches
        //  ret = WriteToPort(hub->GetSwitchState()); // restore old setting NO - let the WOSM do this logic!
