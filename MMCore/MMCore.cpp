...
//                Public License along with the source distribution; if not,
...
//
// NOTES:
//                Public methods follow slightly different naming conventions
//                than the rest of the C++ code, i.e we have:
//                   getConfiguration();
//                instead of:
//                   GetConfiguration();
 ...
 * merging/rebasing.)
 */
const int MMCore_versionMajor = 11, MMCore_versionMinor = 10,
          MMCore_versionPatch = 0;
...
CMMCore::CMMCore()
    : logManager_(new mm::LogManager()),
      ...
      coreLogger_(logManager_->NewLogger("Core")), everSnapped_(false),
      pollingIntervalMs_(10), timeoutMs_(5000), autoShutter_(true),
      callback_(0), configGroups_(0), properties_(0), externalCallback_(0),
      pixelSizeGroup_(0), cbuf_(0), pluginManager_(new CPluginManager()),
 ...
 * - "ParallelDeviceInitialization" (default: enabled) When enabled, serial
 * ports are initialized in serial order, and all other devices are in parallel,
 * using multiple threads, one per device module.  Early testing shows this to
 * be reliable, but switch this off when issues are encountered during device
 * initialization.
 *
 ...
 * disabled feature.
 */
void CMMCore::enableFeature(const char *name, bool enable)
 ...
 */
void CMMCore::setPrimaryLogFile(const char *filename, bool truncate)
    MMCORE_LEGACY_THROW(CMMError) {
...
void CMMCore::logMessage(const char *msg, bool debugOnly) {
  appLogger_(debugOnly ? mm::logging::LogLevelDebug : mm::logging::LogLevelInfo,
             msg);
}

/**
 * Enable or disable logging of debug messages.
 * @param enable   if set to true, debug messages will be recorded in the log
 * file
 */
 ...
 * @param enable     if set to true, log file messages will be echoed on the
 * stderr.
 */
 ...
 */
bool CMMCore::stderrLogEnabled() { return logManager_->IsUsingStdErr(); }
...
int CMMCore::startSecondaryLogFile(const char *filename, bool enableDebug,
                                   bool truncate, bool synchronous)
    MMCORE_LEGACY_THROW(CMMError) {
  if (!filename)
  ...
  LogFileHandle handle = logManager_->AddSecondaryLogFile(
      (enableDebug ? LogLevelTrace : LogLevelInfo), filename, truncate,
...
std::string CMMCore::getVersionInfo() const {
  ...
  std::string debug;
  txt << "MMCore version " << MMCore_versionMajor << "." << MMCore_versionMinor
      << "." << MMCore_versionPatch;
  return txt.str();
}
...
CMMCore::getAvailableDeviceDescriptions(const char *moduleName)
    MMCORE_LEGACY_THROW(CMMError) {
...
std::string CMMCore::getAPIVersionInfo() const {
  std::ostringstream txt;
  txt << "Device API version " << DEVICE_INTERFACE_VERSION << ", "
      << "Module API version " << MODULE_INTERFACE_VERSION;
  return txt.str();
}

/** Returns the MMDevice module interface version number. */
int CMMCore::getMMDeviceModuleInterfaceVersion() {
  return MODULE_INTERFACE_VERSION;
}

/** Returns the MMDevice device interface version number. */
int CMMCore::getMMDeviceDeviceInterfaceVersion() {
  return DEVICE_INTERFACE_VERSION;
}

/**
 * Returns the entire system state, i.e. the collection of all property values
 * from all devices.
 *
 ...
 * value.
 *
 * @return Configuration object containing a collection of device-property-value
 * triplets
 */
Configuration CMMCore::getSystemState() {
  ...
  for (std::vector<std::string>::const_iterator i = devices.begin(),
    ...
    for (std::vector<std::string>::const_iterator it = propertyNames.begin(),
      ...
      config.addSetting(
          PropertySetting(i->c_str(), it->c_str(), val.c_str(), readOnly));
    }
  }
  ...
  for (unsigned i = 0; i < coreProps.size(); i++) {
    ...
    config.addSetting(PropertySetting(MM::g_Keyword_CoreDevice, name.c_str(),
                                      val.c_str(),
                                      properties_->IsReadOnly(name.c_str())));
  }
...
/**
 * Returns the entire system state, i.e. the collection of all property values
 * from all devices. This method will return cached values instead of querying
 * each device
 * @return  Configuration object containing a collection of
 * device-property-value triplets
 */
 ...
 */
Configuration CMMCore::getConfigState(const char *group, const char *config)
    MMCORE_LEGACY_THROW(CMMError) {
  ...
  for (size_t i = 0; i < cfgData.size(); i++) {
    ...
    std::string value =
        getProperty(cs.getDeviceLabel().c_str(), cs.getPropertyName().c_str());
    PropertySetting ss(cs.getDeviceLabel().c_str(),
                       cs.getPropertyName().c_str(),
                       value.c_str()); // state setting
    state.addSetting(ss);
  }
  return state;
}
...
/**
 * Returns the partial state of the system cache, only for the devices included
 * in the specified group. It will create a union of all devices referenced in a
 * group.
 */
 ...
 * specified group. It will create a union of all devices referenced in a group.
 */
Configuration CMMCore::getConfigGroupState(const char *group, bool fromCache)
    MMCORE_LEGACY_THROW(CMMError) {
  ...
  for (std::vector<std::string>::const_iterator it = allPresets.begin(),
                                                end = allPresets.end();
       it != end; ++it) {
    ...
    for (size_t i = 0; i < preset.size(); i++) {
      ...
      if (!state.isPropertyIncluded(deviceLabel.c_str(),
        ...
        if (fromCache) {
          value =
              getPropertyFromCache(deviceLabel.c_str(), propertyName.c_str());
        } else {
          value = getProperty(deviceLabel.c_str(), propertyName.c_str());
        }
 ...
 * if any of the properties fail or if the requested device is not present. It
 * will just quietly continue.
 *
...
void CMMCore::setSystemState(const Configuration &conf) {
  for (unsigned i = 0; i < conf.size(); i++) {
    ...
    if (!s.getReadOnly()) {
      try {
        setProperty(s.getDeviceLabel().c_str(), s.getPropertyName().c_str(),
                    s.getPropertyValue().c_str());
      } catch (CMMError &) {
        // Do not give up yet.
      }
    }
 ...
 * @param moduleName  the name of the device adapter module (short name, not
 * full file name)
 * @param deviceName   the name of the device. The name must correspond to one
 * of the names recognized by the specific plugin library.
 */
void CMMCore::loadDevice(const char *label, const char *moduleName,
  ...
  LOG_DEBUG(coreLogger_) << "Will load device " << deviceName << " from "
                         << moduleName;

  std::shared_ptr<LoadedDeviceAdapter> module =
      pluginManager_->GetDeviceAdapter(moduleName);
  std::shared_ptr<DeviceInstance> pDevice = deviceManager_->LoadDevice(
      module, deviceName, label, this, deviceLogger, coreLogger);
  pDevice->SetCallback(callback_);

  LOG_INFO(coreLogger_) << "Did load device " << deviceName << " from "
                        << moduleName << "; label = " << label;
}
...
void CMMCore::assignDefaultRole(std::shared_ptr<DeviceInstance> pDevice) {
  ...
  // configuration. More complicated configurations will typically override
  // default settings.
  ...
  case MM::CameraDevice:
    currentCameraDevice_ = std::static_pointer_cast<CameraInstance>(pDevice);
    LOG_INFO(coreLogger_) << "Default camera set to " << label;
    break;

  case MM::ShutterDevice:
    currentShutterDevice_ = std::static_pointer_cast<ShutterInstance>(pDevice);
    LOG_INFO(coreLogger_) << "Default shutter set to " << label;
    break;

  case MM::XYStageDevice:
    currentXYStageDevice_ = std::static_pointer_cast<XYStageInstance>(pDevice);
  ...
  case MM::SLMDevice:
    currentSLMDevice_ = std::static_pointer_cast<SLMInstance>(pDevice);
    LOG_INFO(coreLogger_) << "Default SLM set to " << label;
    break;

  case MM::GalvoDevice:
    currentGalvoDevice_ = std::static_pointer_cast<GalvoInstance>(pDevice);
 ...
 */
void CMMCore::unloadDevice(
    const char *label ///< the name of the device to unload
    ) MMCORE_LEGACY_THROW(CMMError) {
...
void CMMCore::reset() MMCORE_LEGACY_THROW(CMMError) {
  ...
  // of course one reason to reset is that some device is not configured
  // correctly, so we need to handle any exception thrown from here
  try {
    waitForSystem();
  } catch (CMMError &) {
  }
...
void CMMCore::initializeAllDevicesSerial() MMCORE_LEGACY_THROW(CMMError) {
  std::vector<std::string> devices = deviceManager_->GetDeviceList();
  LOG_INFO(coreLogger_) << "Will initialize " << devices.size()
                        << " devices (serially)";
  ...
  LOG_INFO(coreLogger_) << "Finished initializing " << devices.size()
                        << " devices";
...
/**
 * Calls Initialize() method for each loaded device.
 * This implementation initializes devices on separate threads, one per device
 * module (adapter). This method also initializes allowed values for core
 * properties, based on the collection of loaded devices.
 */
void CMMCore::initializeAllDevicesParallel() MMCORE_LEGACY_THROW(CMMError) {
  ...
  std::map<std::shared_ptr<LoadedDeviceAdapter>,
           std::vector<std::pair<std::shared_ptr<DeviceInstance>, std::string>>>
      moduleMap;
  ...
  for (size_t i = 0; i < devices.size(); i++) {
    ...
    if (pDevice->GetType() == MM::SerialDevice) {
      ...
      if (moduleMap.find(pAdapter) == moduleMap.end()) {
        ...
        moduleMap.find(pAdapter)->second.push_back(
            make_pair(pDevice, devices[i]));
      }
    }
  }

  // Initialize ports first.  This should be fast, so no need to go parallel
  // (also could not hurt really)
  ...
  // second round, spin up threads to initialize non-port devices, one thread
  // per module
  std::vector<std::future<int>> futures;
  for (auto &moduleDevices : moduleMap) {
    auto f = std::async(std::launch::async, &CMMCore::initializeVectorOfDevices,
                        this, moduleDevices.second);
...
/**
 * This helper function is executed by a single thread, allowing
 * initializeAllDevices to operate multi-threaded. All devices are supposed to
 * originate from the same device adapter
 */
int CMMCore::initializeVectorOfDevices(
    std::vector<std::pair<std::shared_ptr<DeviceInstance>, std::string>>
        devicesLabels) {
...
void CMMCore::updateCoreProperties() MMCORE_LEGACY_THROW(CMMError) {
  ...
  updateCoreProperty(MM::g_Keyword_CoreImageProcessor,
                     MM::ImageProcessorDevice);
...
void CMMCore::updateCoreProperty(const char *propName, MM::DeviceType devType)
    MMCORE_LEGACY_THROW(CMMError) {
...
CMMCore::getDeviceInitializationState(const char *label) const
    MMCORE_LEGACY_THROW(CMMError) {
  std::shared_ptr<DeviceInstance> pDevice = deviceManager_->GetDevice(label);
...
void CMMCore::unloadLibrary(const char *moduleName)
    MMCORE_LEGACY_THROW(CMMError) {
  if (moduleName == 0)
    throw CMMError(errorText_[MMERR_NullPointerException],
  ...
  try {
    ...
    for (it = devices.rbegin(); it != devices.rend(); it++) {
      ...
      if (pDev->GetAdapterModule()->GetName() == moduleName) {
        ...
        } // ignore error; device may already have been unloaded
      }
    }
 ...
 */
void CMMCore::setParentLabel(const char *label, const char *parentLabel)
    MMCORE_LEGACY_THROW(CMMError) {
  if (IsCoreDeviceLabel(label))
    // XXX Should be a throw
 ...
 */
void CMMCore::setDeviceDelayMs(const char *label, double delayMs)
    MMCORE_LEGACY_THROW(CMMError) {
  if (IsCoreDeviceLabel(label))
    return; // ignore
  std::shared_ptr<DeviceInstance> pDevice = deviceManager_->GetDevice(label);
 ...
 */
void CMMCore::waitForDevice(std::shared_ptr<DeviceInstance> pDev)
    MMCORE_LEGACY_THROW(CMMError) {
  ...
  while (true) {
    ...
    if (std::chrono::steady_clock::now() > deadline) {
      ...
      throw CMMError("Wait for device " + ToQuotedString(label) +
                         " timed out after " + ToString(timeoutMs_) + "ms",
...
/**
 * Checks the busy status of the entire system. The system will report busy if
 * any of the devices is busy.
 ...
 * The system will report busy if any of the devices of the specified type are
 * busy.
 *
 * @return true on busy
 ...
 */
void CMMCore::waitForConfig(const char *group, const char *configName)
    MMCORE_LEGACY_THROW(CMMError) {
  CheckConfigGroupName(group);
 ...
 */
void CMMCore::setPosition(const char *label, double position)
    MMCORE_LEGACY_THROW(CMMError) {
  ...
  LOG_DEBUG(coreLogger_) << "Will start absolute move of " << label
                         << " to position " << std::fixed
                         << std::setprecision(5) << position << " um";

  mm::DeviceModuleLockGuard guard(pStage);
  int ret = pStage->SetPositionUm(position);
  if (ret != DEVICE_OK) {
    logError(pStage->GetName().c_str(),
             getDeviceErrorText(ret, pStage).c_str());
    throw CMMError(getDeviceErrorText(ret, pStage).c_str(),
                   MMERR_DEVICE_GENERIC);
  }

   // Update "lastSetPosition" in the state cache
   {
      MMThreadGuard scg(stateCacheLock_);
 ...
 * @param d        the amount to move the stage, in microns (positive or
 * negative)
 */
void CMMCore::setRelativePosition(const char *label, double d)
    MMCORE_LEGACY_THROW(CMMError) {
  std::shared_ptr<StageInstance> pStage =
      deviceManager_->GetDeviceOfType<StageInstance>(label);

  LOG_DEBUG(coreLogger_) << "Will start relative move of " << label
                         << " by offset " << std::fixed << std::setprecision(5)
                         << d << " um";
 ...
 */
void CMMCore::setXYPosition(const char *label, double x, double y)
    MMCORE_LEGACY_THROW(CMMError) {
  ...
  LOG_DEBUG(coreLogger_) << "Will start absolute move of " << label
                         << " to position (" << std::fixed
                         << std::setprecision(3) << x << ", " << y << ") um";

  mm::DeviceModuleLockGuard guard(pXYStage);
  int ret = pXYStage->SetPositionUm(x, y);
  if (ret != DEVICE_OK) {
    logError(pXYStage->GetName().c_str(),
             getDeviceErrorText(ret, pXYStage).c_str());
    throw CMMError(getDeviceErrorText(ret, pXYStage).c_str(),
                   MMERR_DEVICE_GENERIC);
  }

  // Cache the new XY position
  {
    MMThreadGuard scg(stateCacheLock_);
    stateCache_.addSetting(PropertySetting(label,
 ...
 */
void CMMCore::setRelativeXYPosition(const char *label, double dx, double dy)
    MMCORE_LEGACY_THROW(CMMError) {
  std::shared_ptr<XYStageInstance> pXYStage =
      deviceManager_->GetDeviceOfType<XYStageInstance>(label);

  LOG_DEBUG(coreLogger_) << "Will start relative move of " << label << " by ("
                         << std::fixed << std::setprecision(3) << dx << ", "
                         << dy << ") um";
 ...
 * @param dy     the distance to move in Y (positive or negative)
 */
void CMMCore::setRelativeXYPosition(double dx, double dy)
    MMCORE_LEGACY_THROW(CMMError) {
 ...
 */
void CMMCore::getXYPosition(const char *label, double &x, double &y)
    MMCORE_LEGACY_THROW(CMMError) {
  std::shared_ptr<XYStageInstance> pXYStage =
      deviceManager_->GetDeviceOfType<XYStageInstance>(label);
 ...
 * @param y            a return parameter yielding the Y position in microns
 */
void CMMCore::getXYPosition(double &x, double &y)
    MMCORE_LEGACY_THROW(CMMError) {
...
void CMMCore::stop(const char *label) MMCORE_LEGACY_THROW(CMMError) {
  std::shared_ptr<DeviceInstance> stage = deviceManager_->GetDevice(label);
  ...
  throw CMMError("Cannot stop " + ToQuotedString(label) + ": not a stage");
}
...
void CMMCore::home(const char *label) MMCORE_LEGACY_THROW(CMMError) {
  ...
  throw CMMError("Cannot home " + ToQuotedString(label) + ": not a stage");
}
...
void CMMCore::setOriginXY(const char *label) MMCORE_LEGACY_THROW(CMMError) {
  ...
  LOG_DEBUG(coreLogger_) << "Zeroed xy stage " << label
   ...
   // Update "lastSetPositionX" and "lastSetPositionY" in the state cache
   {
      MMThreadGuard scg(stateCacheLock_);
      stateCache_.addSetting(PropertySetting(label,
                                             MM::g_Keyword_LastSetPositionX,
...
void CMMCore::setOriginX(const char *label) MMCORE_LEGACY_THROW(CMMError) {
  ...
  LOG_DEBUG(coreLogger_) << "Zeroed x coordinate of xy stage " << label
   ...
   // Update "lastSetPositionX" in the state cache
   {
      MMThreadGuard scg(stateCacheLock_);
      stateCache_.addSetting(PropertySetting(label,
                                             MM::g_Keyword_LastSetPositionX,
...
void CMMCore::setOriginY(const char *label) MMCORE_LEGACY_THROW(CMMError) {
  ...
  LOG_DEBUG(coreLogger_) << "Zeroed y coordinate of xy stage " << label
   ...
   // Update "lastSetPositionY" in the state cache
   {
      MMThreadGuard scg(stateCacheLock_);
      stateCache_.addSetting(PropertySetting(label,
                                             MM::g_Keyword_LastSetPositionY,
 ...
 */
void CMMCore::setAdapterOrigin(const char *label, double newZUm)
    MMCORE_LEGACY_THROW(CMMError) {
  std::shared_ptr<StageInstance> pStage =
      deviceManager_->GetDeviceOfType<StageInstance>(label);
  ...
  LOG_DEBUG(coreLogger_) << "Adapter-zeroed stage " << label
                         << ", assigning coordinate " << std::fixed
                         << std::setprecision(5) << newZUm
                         << " um to the current position";
}
 ...
 */
void CMMCore::setAdapterOriginXY(const char *label, double newXUm,
                                 double newYUm) MMCORE_LEGACY_THROW(CMMError) {
  std::shared_ptr<XYStageInstance> pXYStage =
      deviceManager_->GetDeviceOfType<XYStageInstance>(label);
  ...
  LOG_DEBUG(coreLogger_) << "Adapter-zeroed XY stage " << label
                         << ", assigning coordinates (" << std::fixed
                         << std::setprecision(3) << newXUm << ", " << newYUm
                         << ") um to the current position";
}
 ...
 * @param newYUm   the new coordinate to assign to the current Y position
 */
void CMMCore::setAdapterOriginXY(double newXUm, double newYUm)
    MMCORE_LEGACY_THROW(CMMError) {
 ...
 * This should only be called for cameras where exposure time is sequenceable
 * @param cameraLabel      the camera device label
 * @param exposureTime_ms  sequence of exposure times the camera will use during
 * a sequence acquisition
 */
void CMMCore::loadExposureSequence(const char *cameraLabel,
  ...
  if (exposureTime_ms.size() > maxLength) {
    throw CMMError("The length of the requested exposure sequence (" +
                   ToString(exposureTime_ms.size()) +
 ...
 * @param label              the device label
 * @param positionSequence   a sequence of positions that the stage will execute
 ...
 */
void CMMCore::setStageLinearSequence(const char *label, double dZ_um,
                                     int nSlices)
    MMCORE_LEGACY_THROW(CMMError) {
  if (nSlices < 0)
 ...
 * This should only be called for XY stages that are sequenceable
 * @param label        the XY stage device label
 * @param xSequence    the sequence of x positions that the stage will execute
 * in response to external triggers
 * @param ySequence    the sequence of y positions that the stage will execute
 * in response to external triggers
 */
  ...
  if (camera) {
    ...
    try {
      ...
      if (autoShutter_ && shutter) {
        ...
        if (DEVICE_OK != sret) {
          logError("CMMCore::snapImage",
                   getDeviceErrorText(sret, shutter).c_str());
          throw CMMError(getDeviceErrorText(sret, shutter).c_str(),
                         MMERR_DEVICE_GENERIC);
        }
      ...
      logError("CMMCore::snapImage",
               getCoreErrorText(MMERR_UnhandledException).c_str());
      throw CMMError(getCoreErrorText(MMERR_UnhandledException).c_str(),
                     MMERR_UnhandledException);
    }

    if (ret != DEVICE_OK) {
      logError("CMMCore::snapImage", getDeviceErrorText(ret, camera).c_str());
      throw CMMError(getDeviceErrorText(ret, camera).c_str(),
                     MMERR_DEVICE_GENERIC);
    }
  } else {
...
void CMMCore::setAutoShutter(bool state) {
  ...
  {
    MMThreadGuard scg(stateCacheLock_);
    stateCache_.addSetting(PropertySetting(MM::g_Keyword_CoreDevice,
                                           MM::g_Keyword_CoreAutoShutter,
                                           state ? "1" : "0"));
  }
 ...
 */
bool CMMCore::getAutoShutter() { return autoShutter_; }
 ...
 */
void CMMCore::setShutterOpen(const char *shutterLabel, bool state)
    MMCORE_LEGACY_THROW(CMMError) {
  ...
  if (pShutter) {
    ...
    if (ret != DEVICE_OK) {
      logError("CMMCore::setShutterOpen()",
               getDeviceErrorText(ret, pShutter).c_str());
      throw CMMError(getDeviceErrorText(ret, pShutter).c_str(),
                     MMERR_DEVICE_GENERIC);
    }
    ...
    if (pShutter->HasProperty(MM::g_Keyword_State)) {
      {
        MMThreadGuard scg(stateCacheLock_);
        stateCache_.addSetting(
            PropertySetting(shutterLabel, MM::g_Keyword_State,
...
bool CMMCore::getShutterOpen(const char *shutterLabel)
  ...
  if (pShutter) {
    ...
    if (ret != DEVICE_OK) {
      logError("CMMCore::getShutterOpen()",
               getDeviceErrorText(ret, pShutter).c_str());
      throw CMMError(getDeviceErrorText(ret, pShutter).c_str(),
                     MMERR_DEVICE_GENERIC);
    }
 ...
 * RGB buffers are expected to be in big endian ARGB format (ARGB8888), which
 * means that on little endian the format is BGRA888 (see:
...
void *CMMCore::getImage() MMCORE_LEGACY_THROW(CMMError) {
  ...
  if (!camera)
    ...
    if (!everSnapped_) {
      logError("CMMCore::getImage()",
               getCoreErrorText(MMERR_InvalidImageSequence).c_str());
      throw CMMError(getCoreErrorText(MMERR_InvalidImageSequence).c_str(),
                     MMERR_InvalidImageSequence);
    }
    ...
    try {
      ...
      if (imageProcessor) {
        imageProcessor->Process((unsigned char *)pBuf, camera->GetImageWidth(),
    ...
    if (pBuf != 0)
      ...
      logError("CMMCore::getImage()",
               getCoreErrorText(MMERR_CameraBufferReadFailed).c_str());
      throw CMMError(getCoreErrorText(MMERR_CameraBufferReadFailed).c_str(),
                     MMERR_CameraBufferReadFailed);
    }
  }
}
 ...
 * @param intervalMs       The interval between images, currently only supported
 * by Andor cameras
 * @param stopOnOverflow   whether or not the camera stops acquiring when the
 * circular buffer is full
 */
void CMMCore::startSequenceAcquisition(long numImages, double intervalMs,
                                       bool stopOnOverflow)
    MMCORE_LEGACY_THROW(CMMError) {
  std::shared_ptr<CameraInstance> camera = currentCameraDevice_.lock();
  if (camera) {
    ...
    try {
      if (!cbuf_->Initialize(camera->GetNumberOfChannels(),
                             camera->GetImageWidth(), camera->GetImageHeight(),
                             camera->GetImageBytesPerPixel())) {
        logError(
            getDeviceName(camera).c_str(),
      ...
      LOG_DEBUG(coreLogger_)
          << "Will start sequence acquisition from default camera";
      int nRet = camera->StartSequenceAcquisition(numImages, intervalMs,
                                                  stopOnOverflow);
      if (nRet != DEVICE_OK)
    ...
    logError(getDeviceName(camera).c_str(),
             getCoreErrorText(MMERR_CameraNotAvailable).c_str());
    throw CMMError(getCoreErrorText(MMERR_CameraNotAvailable).c_str(),
                   MMERR_CameraNotAvailable);
  }
  LOG_DEBUG(coreLogger_)
      << "Did start sequence acquisition from default camera";
 ...
 * This command does not block the calling thread for the duration of the
 * acquisition. The difference between this method and the one with the same
 * name but operating on the "default" camera is that it does not automatically
 * initialize the circular buffer.
 */
void CMMCore::startSequenceAcquisition(const char *label, long numImages,
                                       double intervalMs, bool stopOnOverflow)
    MMCORE_LEGACY_THROW(CMMError) {
  ...
  if (!cbuf_->Initialize(pCam->GetNumberOfChannels(), pCam->GetImageWidth(),
                         pCam->GetImageHeight(),
                         pCam->GetImageBytesPerPixel())) {
    logError(getDeviceName(pCam).c_str(),
             getCoreErrorText(MMERR_CircularBufferFailedToInitialize).c_str());
    throw CMMError(
        getCoreErrorText(MMERR_CircularBufferFailedToInitialize).c_str(),
        MMERR_CircularBufferFailedToInitialize);
  ...
  cbuf_->Clear();
  cbuf_->SetOverwriteData(!stopOnOverflow);
  LOG_DEBUG(coreLogger_) << "Will start sequence acquisition from camera "
                         << label;
  int nRet =
      pCam->StartSequenceAcquisition(numImages, intervalMs, stopOnOverflow);
  if (nRet != DEVICE_OK)
    throw CMMError(getDeviceErrorText(nRet, pCam).c_str(),
                   MMERR_DEVICE_GENERIC);

  LOG_DEBUG(coreLogger_) << "Did start sequence acquisition from camera "
                         << label;
  // onSequenceAcquisitionStarted will be called by CoreCallback::PrepareForAcq
}
...
void CMMCore::initializeCircularBuffer() MMCORE_LEGACY_THROW(CMMError) {
  ...
  LOG_DEBUG(coreLogger_)
      << "Circular buffer initialized based on current camera";
}
...
void CMMCore::stopSequenceAcquisition(const char *label)
  ...
  mm::DeviceModuleLockGuard guard(pCam);
  LOG_DEBUG(coreLogger_) << "Will stop sequence acquisition from camera "
                         << label;
  ...
  LOG_DEBUG(coreLogger_) << "Did stop sequence acquisition from camera "
                         << label;
...
void CMMCore::startContinuousSequenceAcquisition(double intervalMs)
  ...
  if (camera) {
    ...
    LOG_DEBUG(coreLogger_)
        << "Will start continuous sequence acquisition from current camera";
  ...
  LOG_DEBUG(coreLogger_)
      << "Did start continuous sequence acquisition from current camera";
  // onSequenceAcquisitionStarted will be called by CoreCallback::PrepareForAcq
}
...
void *CMMCore::getLastImage() MMCORE_LEGACY_THROW(CMMError) {
  ...
  if (pBuf != 0)
    ...
    logError("CMMCore::getLastImage",
             getCoreErrorText(MMERR_CircularBufferEmpty).c_str());
    throw CMMError(getCoreErrorText(MMERR_CircularBufferEmpty).c_str(),
                   MMERR_CircularBufferEmpty);
  }
}

void *CMMCore::getLastImageMD(unsigned channel, unsigned slice,
                              Metadata &md) const
    MMCORE_LEGACY_THROW(CMMError) {
 ...
 * https://en.wikipedia.org/wiki/RGBA_color_model).
 */
void *CMMCore::getNBeforeLastImageMD(unsigned long n, Metadata &md) const
    MMCORE_LEGACY_THROW(CMMError) {
 ...
 */
void *CMMCore::popNextImageMD(unsigned channel, unsigned slice, Metadata &md)
    MMCORE_LEGACY_THROW(CMMError) {
  // Slices have never been implemented on the device interface side
  if (slice != 0)
    throw CMMError("Slice must be 0");
...
void CMMCore::setCircularBufferMemoryFootprint(unsigned sizeMB ///< n megabytes
  ...
  LOG_DEBUG(coreLogger_) << "Will set circular buffer size to " << sizeMB
                         << " MB";
  try {
  ...
  if (NULL == cbuf_)
    throw CMMError(getCoreErrorText(MMERR_OutOfMemory).c_str(),
                   MMERR_OutOfMemory);
  ...
  try {
    ...
    LOG_DEBUG(coreLogger_) << "Did set circular buffer size to " << sizeMB
                           << " MB";
  } catch (std::bad_alloc &ex) {
    std::ostringstream messs;
    messs << getCoreErrorText(MMERR_OutOfMemory).c_str() << " " << ex.what()
 ...
 */
bool CMMCore::isBufferOverflowed() const { return cbuf_->Overflow(); }
...
std::string CMMCore::getAutoFocusDevice() {
  std::shared_ptr<AutoFocusInstance> autofocus = currentAutofocusDevice_.lock();
  if (autofocus) {
...
void CMMCore::setAutoFocusDevice(const char *autofocusLabel)
  ...
  properties_->Set(MM::g_Keyword_CoreAutoFocus, newAutofocusLabel.c_str());
  {
    MMThreadGuard scg(stateCacheLock_);
    stateCache_.addSetting(PropertySetting(MM::g_Keyword_CoreDevice,
                                           MM::g_Keyword_CoreAutoFocus,
                                           newAutofocusLabel.c_str()));
  }
}
...
void CMMCore::setImageProcessorDevice(const char *procLabel)
  ...
  properties_->Set(MM::g_Keyword_CoreImageProcessor, newProcLabel.c_str());
  {
    MMThreadGuard scg(stateCacheLock_);
    stateCache_.addSetting(PropertySetting(MM::g_Keyword_CoreDevice,
                                           MM::g_Keyword_CoreImageProcessor,
                                           newProcLabel.c_str()));
  }
}
...
void CMMCore::setSLMDevice(const char *slmLabel) MMCORE_LEGACY_THROW(CMMError) {
  if (slmLabel && strlen(slmLabel) > 0) {
    currentSLMDevice_ = deviceManager_->GetDeviceOfType<SLMInstance>(slmLabel);
  ...
  {
    MMThreadGuard scg(stateCacheLock_);
    stateCache_.addSetting(PropertySetting(
        MM::g_Keyword_CoreDevice, MM::g_Keyword_CoreSLM, newSLMLabel.c_str()));
  }
}
 ...
 */
std::string CMMCore::getChannelGroup() { return channelGroup_; }
...
void CMMCore::setShutterDevice(const char *shutterLabel)
  ...
  std::shared_ptr<ShutterInstance> oldShutter = currentShutterDevice_.lock();
  if (oldShutter) {
  ...
  properties_->Set(MM::g_Keyword_CoreShutter, newShutterLabel.c_str());
  {
    MMThreadGuard scg(stateCacheLock_);
    stateCache_.addSetting(PropertySetting(MM::g_Keyword_CoreDevice,
                                           MM::g_Keyword_CoreShutter,
                                           newShutterLabel.c_str()));
  }
}
...
void CMMCore::setXYStageDevice(const char *xyDeviceLabel)
  ...
  properties_->Set(MM::g_Keyword_CoreXYStage, newXYStageLabel.c_str());
  {
    MMThreadGuard scg(stateCacheLock_);
    stateCache_.addSetting(PropertySetting(MM::g_Keyword_CoreDevice,
                                           MM::g_Keyword_CoreXYStage,
                                           newXYStageLabel.c_str()));
  }
}
...
void CMMCore::setCameraDevice(const char *cameraLabel)
  ...
  properties_->Set(MM::g_Keyword_CoreCamera, newCameraLabel.c_str());
  {
    MMThreadGuard scg(stateCacheLock_);
    stateCache_.addSetting(PropertySetting(MM::g_Keyword_CoreDevice,
                                           MM::g_Keyword_CoreCamera,
                                           newCameraLabel.c_str()));
  }
}
 ...
 */
std::vector<std::string>
CMMCore::getLoadedDevicesOfType(MM::DeviceType devType) const {
 ...
 * However, even if all values are allowed it is not guaranteed that all of them
 * will be actually accepted by the device at run time.
 *
 ...
 * @param propName   the property name
 */
std::string CMMCore::getProperty(const char *label, const char *propName)
    MMCORE_LEGACY_THROW(CMMError) {
  if (IsCoreDeviceLabel(label))
  ...
  // Note, stateCache is mutable so that we can update it from this const
  // function
...
void CMMCore::setProperty(const char *label, const char *propName,
  ...
  if (IsCoreDeviceLabel(label)) {
    LOG_DEBUG(coreLogger_) << "Will set Core property: " << propName << " = "
                           << propValue;
    ...
    properties_->Execute(propName, propValue);
    {
      MMThreadGuard scg(stateCacheLock_);
      stateCache_.addSetting(
          PropertySetting(MM::g_Keyword_CoreDevice, propName, propValue));
    }

    LOG_DEBUG(coreLogger_) << "Did set Core property: " << propName << " = "
                           << propValue;
  } else {
    std::shared_ptr<DeviceInstance> pDevice = deviceManager_->GetDevice(label);
 ...
 */
bool CMMCore::hasProperty(const char *label, const char *propName)
    MMCORE_LEGACY_THROW(CMMError) {
  if (IsCoreDeviceLabel(label))
 ...
 * @param propName the property name
 */
bool CMMCore::isPropertyReadOnly(const char *label, const char *propName)
    MMCORE_LEGACY_THROW(CMMError) {
  if (IsCoreDeviceLabel(label))
 ...
 * @param propName   the property name
 */
bool CMMCore::isPropertyPreInit(const char *label, const char *propName)
    MMCORE_LEGACY_THROW(CMMError) {
  if (IsCoreDeviceLabel(label))
    return false;
  std::shared_ptr<DeviceInstance> pDevice = deviceManager_->GetDevice(label);
 ...
 * otherwise.
 */
double CMMCore::getPropertyLowerLimit(const char *label, const char *propName)
    MMCORE_LEGACY_THROW(CMMError) {
  if (IsCoreDeviceLabel(label))
    return 0.0;
  std::shared_ptr<DeviceInstance> pDevice = deviceManager_->GetDevice(label);
 ...
 * otherwise.
 */
double CMMCore::getPropertyUpperLimit(const char *label, const char *propName)
    MMCORE_LEGACY_THROW(CMMError) {
  if (IsCoreDeviceLabel(label))
    return 0.0;
  std::shared_ptr<DeviceInstance> pDevice = deviceManager_->GetDevice(label);
 ...
 */
bool CMMCore::hasPropertyLimits(const char *label, const char *propName)
    MMCORE_LEGACY_THROW(CMMError) {
  if (IsCoreDeviceLabel(label))
    return false;
  std::shared_ptr<DeviceInstance> pDevice = deviceManager_->GetDevice(label);
 ...
 * @param propName   the property label
 */
bool CMMCore::isPropertySequenceable(const char *label, const char *propName)
    MMCORE_LEGACY_THROW(CMMError) {
  if (IsCoreDeviceLabel(label))
    return false;
  std::shared_ptr<DeviceInstance> pDevice = deviceManager_->GetDevice(label);
...
/**
 * Queries device property for the maximum number of events that can be put in a
 * sequence
 * @param label      the device name
 * @param propName   the property label
 */
 ...
 * @param propName   the property label
 */
void CMMCore::startPropertySequence(const char *label, const char *propName)
    MMCORE_LEGACY_THROW(CMMError) {
  if (IsCoreDeviceLabel(label))
    // XXX Should be a throw
    return;
 ...
 * @param propName  the property name
 */
void CMMCore::stopPropertySequence(const char *label, const char *propName)
    MMCORE_LEGACY_THROW(CMMError) {
  if (IsCoreDeviceLabel(label))
    // XXX Should be a throw
    return;
 ...
 * @param label           the device name
 * @param propName        the property label
 * @param eventSequence   the sequence of events/states that the device will
 * execute in response to external triggers
 */
void CMMCore::loadPropertySequence(const char *label, const char *propName,
...
/**
 * How many bits of dynamic range are to be expected from the camera. This value
 * should be used only as a guideline - it does not guarantee that image buffer
 * will contain only values from the returned dynamic range.
 *
 ...
 * @param dExp   the exposure in milliseconds
 */
void CMMCore::setExposure(const char *label, double dExp)
    MMCORE_LEGACY_THROW(CMMError) {
  std::shared_ptr<CameraInstance> pCamera =
      deviceManager_->GetDeviceOfType<CameraInstance>(label);
  ...
  {
    mm::DeviceModuleLockGuard guard(pCamera);
    LOG_DEBUG(coreLogger_) << "Will set camera " << label << " exposure to "
                           << std::fixed << std::setprecision(3) << dExp
                           << " ms";
    ...
    if (pCamera->HasProperty(MM::g_Keyword_Exposure)) {
      {
        MMThreadGuard scg(stateCacheLock_);
        stateCache_.addSetting(
            PropertySetting(label, MM::g_Keyword_Exposure,
                            CDeviceUtils::ConvertToString(dExp)));
      }
    }
  }

  LOG_DEBUG(coreLogger_) << "Did set camera " << label << " exposure to "
                         << std::fixed << std::setprecision(3) << dExp << " ms";
}
 ...
 */
void CMMCore::setROI(int x, int y, int xSize, int ySize)
    MMCORE_LEGACY_THROW(CMMError) {
  std::shared_ptr<CameraInstance> camera = currentCameraDevice_.lock();
  if (camera) {
    mm::DeviceModuleLockGuard guard(camera);
 ...
 * @param ySize  number of horizontal pixels
 */
void CMMCore::getROI(int &x, int &y, int &xSize, int &ySize)
    MMCORE_LEGACY_THROW(CMMError) {
 ...
 * @param ySize  number of horizontal pixels
 */
void CMMCore::setROI(const char *label, int x, int y, int xSize, int ySize)
    MMCORE_LEGACY_THROW(CMMError) {
 ...
 */
void CMMCore::getROI(const char *label, int &x, int &y, int &xSize, int &ySize)
    MMCORE_LEGACY_THROW(CMMError) {
  std::shared_ptr<CameraInstance> pCam =
      deviceManager_->GetDeviceOfType<CameraInstance>(label);
...
void CMMCore::setMultiROI(std::vector<unsigned> xs, std::vector<unsigned> ys,
    ...
    MMCORE_LEGACY_THROW(CMMError) {
  if (xs.size() != ys.size() || xs.size() != widths.size() ||
  ...
  int nRet = camera->SetMultiROI(xs.data(), ys.data(), widths.data(),
                                 heights.data(), numROI);
  if (nRet != DEVICE_OK) {
    throw CMMError(getDeviceErrorText(nRet, camera).c_str(),
                   MMERR_DEVICE_GENERIC);
...
void CMMCore::getMultiROI(std::vector<unsigned> &xs, std::vector<unsigned> &ys,
  ...
  nRet = camera->GetMultiROI(xsTmp.data(), ysTmp.data(), widthsTmp.data(),
                             heightsTmp.data(), &newNum);
  if (nRet != DEVICE_OK) {
    throw CMMError(getDeviceErrorText(nRet, camera).c_str(),
                   MMERR_DEVICE_GENERIC);
  }
 ...
 */
void CMMCore::setState(const char *deviceLabel, long state)
    MMCORE_LEGACY_THROW(CMMError) {
  ...
  if (pStateDev->HasProperty(MM::g_Keyword_State)) {
    {
      MMThreadGuard scg(stateCacheLock_);
      stateCache_.addSetting(
          PropertySetting(deviceLabel, MM::g_Keyword_State,
                          CDeviceUtils::ConvertToString(state)));
    }
  }
  if (pStateDev->HasProperty(MM::g_Keyword_Label)) {
...

    {
      MMThreadGuard scg(stateCacheLock_);
      stateCache_.addSetting(
          PropertySetting(deviceLabel, MM::g_Keyword_Label, posLbl.c_str()));
    }
  }
...
/**
 * Returns the current state (position) on the specific device. The command will
 * fail if the device does not support states.
 *
 ...
 */
void CMMCore::setStateLabel(const char *deviceLabel, const char *stateLabel)
    MMCORE_LEGACY_THROW(CMMError) {
  std::shared_ptr<StateInstance> pStateDev =
      deviceManager_->GetDeviceOfType<StateInstance>(deviceLabel);
  ...
  mm::DeviceModuleLockGuard guard(pStateDev);
  LOG_DEBUG(coreLogger_) << "Will set " << deviceLabel << " to label "
                         << stateLabel;
  int nRet = pStateDev->SetPosition(stateLabel);
  if (nRet != DEVICE_OK)
    throw CMMError(getDeviceErrorText(nRet, pStateDev));
  LOG_DEBUG(coreLogger_) << "Did set " << deviceLabel << " to label "
                         << stateLabel;
  ...
  if (pStateDev->HasProperty(MM::g_Keyword_Label)) {
    {
      MMThreadGuard scg(stateCacheLock_);
      stateCache_.addSetting(
          PropertySetting(deviceLabel, MM::g_Keyword_Label, stateLabel));
    }
  }
  if (pStateDev->HasProperty(MM::g_Keyword_State)) {
 ...
 */
void CMMCore::defineStateLabel(const char *deviceLabel, long state,
                               const char *label)
    MMCORE_LEGACY_THROW(CMMError) {
  std::shared_ptr<StateInstance> pStateDev =
      deviceManager_->GetDeviceOfType<StateInstance>(deviceLabel);
  ...
  if (label != oldLabel) {
    ...
    while (itcfg != configGroups.end()) {
      ...
      while (itcf != configs.end()) {
        ...
        if (!oldLabel.empty() &&
            conf.isPropertyIncluded(deviceLabel, MM::g_Keyword_Label)) {
          PropertySetting setting(deviceLabel, MM::g_Keyword_Label,
                                  oldLabel.c_str());
          if (conf.isSettingIncluded(setting)) {
            deleteConfig((*itcfg).c_str(), (*itcf).c_str(), deviceLabel,
                         MM::g_Keyword_Label);
            defineConfig((*itcfg).c_str(), (*itcf).c_str(), deviceLabel,
                         MM::g_Keyword_Label, label);
          }
        }
        itcf++;
      }
  ...
  LOG_DEBUG(coreLogger_) << "Defined label " << label << " for device "
                         << deviceLabel << " state " << state;
}
 ...
 */
long CMMCore::getStateFromLabel(const char *deviceLabel, const char *stateLabel)
    MMCORE_LEGACY_THROW(CMMError) {
  std::shared_ptr<StateInstance> pStateDev =
      deviceManager_->GetDeviceOfType<StateInstance>(deviceLabel);
  CheckStateLabel(stateLabel);
...
void CMMCore::defineConfigGroup(const char *groupName)
  ...
  if (!configGroups_->Define(groupName))
    throw CMMError(ToQuotedString(groupName) + ": " +
                       getCoreErrorText(MMERR_DuplicateConfigGroup),
...
void CMMCore::deleteConfigGroup(const char *groupName)
  ...
  if (!configGroups_->Delete(groupName))
    throw CMMError(ToQuotedString(groupName) + ": " +
                       getCoreErrorText(MMERR_NoConfigGroup),
                   MMERR_NoConfigGroup);
...
void CMMCore::renameConfigGroup(const char *oldGroupName,
  ...
  if (!configGroups_->RenameGroup(oldGroupName, newGroupName))
    throw CMMError(ToQuotedString(oldGroupName) + ": " +
                       getCoreErrorText(MMERR_NoConfigGroup),
                   MMERR_NoConfigGroup);

  LOG_DEBUG(coreLogger_) << "Renamed config group " << oldGroupName << " to "
                         << newGroupName;
...
/**
 * Defines a configuration. If the configuration group/name was not previously
 * defined a new configuration will be automatically created; otherwise nothing
 * happens.
 *
 * @param groupName    the configuration group name
 * @param configName   the configuration preset name
 */
void CMMCore::defineConfig(const char *groupName, const char *configName)
    MMCORE_LEGACY_THROW(CMMError) {
  CheckConfigGroupName(groupName);
  CheckConfigPresetName(configName);
  ...
  LOG_DEBUG(coreLogger_) << "Config group " << groupName << ": added preset "
                         << configName;
}

/**
 * Defines a single configuration entry (setting). If the configuration
 * group/name was not previously defined a new configuration will be
 * automatically created. If the name was previously defined the new setting
 * will be added to its list of property settings. The new setting will override
 * previously defined ones if it refers to the same property name.
 *
...
void CMMCore::defineConfig(const char *groupName, const char *configName,
  ...
  LOG_DEBUG(coreLogger_) << "Config group " << groupName << ": preset "
                         << configName << ": added setting " << deviceLabel
                         << "-" << propName << " = " << value;
}
 ...
 * presets, i.e. it will try to detect if any of the pixel size presets matches
 * the current state of the system. If the pixel size was previously defined the
 * new setting will be added to its list of property settings. The new setting
 * will override previously defined ones if it refers to the same property name.
 *
...
void CMMCore::definePixelSizeConfig(const char *resolutionID,
  ...
  LOG_DEBUG(coreLogger_) << "Pixel size config: "
                            "preset "
                         << resolutionID << ": added setting : " << deviceLabel
                         << "-" << propName << " = " << value;
}
...
/**
 * Sets pixel size in microns for the specified resolution sensing configuration
 * preset.
 */
void CMMCore::setPixelSizeUm(const char *resolutionID, double pixSize)
    MMCORE_LEGACY_THROW(CMMError) {
  CheckConfigPresetName(resolutionID);

  PixelSizeConfiguration *psc = pixelSizeGroup_->Find(resolutionID);
  if (psc == 0)
    throw CMMError(ToQuotedString(resolutionID) + ": " +
                       getCoreErrorText(MMERR_NoConfigGroup),
                   MMERR_NoConfigGroup);
  ...
  LOG_DEBUG(coreLogger_) << "Pixel size config: "
                            "preset "
                         << resolutionID << ": set resolution to " << std::fixed
                         << std::setprecision(5) << pixSize << " um/px";
}
...
void CMMCore::setPixelSizeAffine(const char *resolutionID,
  ...
  LOG_DEBUG(coreLogger_) << "Pixel size config: "
                            "preset "
                         << resolutionID << ": set affine matrix to "
                         << std::fixed << std::setprecision(5) << affine[0]
                         << ", " << std::fixed << std::setprecision(5)
                         << affine[1] << ", " << std::fixed
                         << std::setprecision(5) << affine[2] << ", "
                         << std::fixed << std::setprecision(5) << affine[3]
                         << ", " << std::fixed << std::setprecision(5)
                         << affine[4] << ", " << std::fixed
                         << std::setprecision(5) << affine[5];
}
 ...
 * @param dxdz       Angle of the Z-stage axis with the camera axis
 * (dimensionless)
 */
void CMMCore::setPixelSizedxdz(const char *resolutionID, double dxdz)
    MMCORE_LEGACY_THROW(CMMError) {
  CheckConfigPresetName(resolutionID);
  ...
  LOG_DEBUG(coreLogger_) << "Pixel size config: "
                            "preset "
                         << resolutionID << ": set dxdz to " << std::fixed
                         << std::setprecision(5) << dxdz;
}
 ...
 *
 * @param resolutionID   The pixel size configuration group name
 * @param dydz       Angle of the Z-stage axis with the camera axis
 * (dimensionless)
 */
void CMMCore::setPixelSizedydz(const char *resolutionID, double dydz)
    MMCORE_LEGACY_THROW(CMMError) {
  CheckConfigPresetName(resolutionID);
  ...
  LOG_DEBUG(coreLogger_) << "Pixel size config: "
                            "preset "
                         << resolutionID << ": set dydz to " << std::fixed
                         << std::setprecision(5) << dydz;
}
 ...
 */
void CMMCore::setPixelSizeOptimalZUm(const char *resolutionID, double optimalZ)
    MMCORE_LEGACY_THROW(CMMError) {
  CheckConfigPresetName(resolutionID);
  ...
  LOG_DEBUG(coreLogger_) << "Pixel size config: "
                            "preset "
                         << resolutionID << ": set optimalZ to " << std::fixed
                         << std::setprecision(5) << optimalZ << " um.";
}
...
void CMMCore::setPixelSizeConfig(const char *resolutionID)
  ...
  if (!psc) {
    throw CMMError(ToQuotedString(resolutionID) + ": " +
                       getCoreErrorText(MMERR_NoConfiguration),
 ...
 * @param configName  the configuration preset name
 */
void CMMCore::setConfig(const char *groupName, const char *configName)
    MMCORE_LEGACY_THROW(CMMError) {
  CheckConfigGroupName(groupName);
  CheckConfigPresetName(configName);
...
/**
 * Renames a configuration within a specified group. The command will fail if
 * the configuration was not previously defined.
 *
 */
void CMMCore::renameConfig(const char *groupName, const char *oldConfigName,
  ...
  LOG_DEBUG(coreLogger_) << "Config group " << groupName << ": renamed preset "
                         << oldConfigName << " to " << newConfigName;
}
 ...
 *
 */
void CMMCore::deleteConfig(const char *groupName, const char *configName)
    MMCORE_LEGACY_THROW(CMMError) {
  CheckConfigGroupName(groupName);
  CheckConfigPresetName(configName);
  ...
  LOG_DEBUG(coreLogger_) << "Config group " << groupName << ": deleted preset "
                         << configName;
}

/**
 * Deletes a property from a configuration in the specified group. The command
 * will fail if the configuration was not previously defined.
 *
 */
void CMMCore::deleteConfig(const char *groupName, const char *configName,
                           const char *deviceLabel, const char *propName)
  ...
  if (!configGroups_->Delete(groupName, configName, deviceLabel, propName)) {
    logError("deleteConfig", getCoreErrorText(MMERR_NoConfiguration).c_str());
    throw CMMError("Property " + ToQuotedString(propName) + " of device " +
                       ToQuotedString(deviceLabel) + " is not in preset " +
                       ToQuotedString(configName) + " of configuration group " +
                       ToQuotedString(groupName),
                   MMERR_NoConfiguration);
  }

  LOG_DEBUG(coreLogger_) << "Config group " << groupName << ": preset "
                         << configName << ": deleted property " << deviceLabel
                         << "-" << propName;
}
 ...
 * Also, in general it is possible that the system state fits multiple
 * configurations. This method will return only the first matching
...
Configuration CMMCore::getPixelSizeConfigData(const char *configName)
  ...
  if (!pCfg) {
    ...
    logError(os.str().c_str(), getCoreErrorText(MMERR_NoConfiguration).c_str());
    throw CMMError("Pixel size configuration preset " +
                       ToQuotedString(configName) + " does not exist",
                   MMERR_NoConfiguration);
  }
  return *pCfg;
}
...
void CMMCore::renamePixelSizeConfig(const char *oldConfigName,
  ...
  if (!pixelSizeGroup_->Rename(oldConfigName, newConfigName)) {
    logError("renamePixelSizeConfig",
             getCoreErrorText(MMERR_NoConfiguration).c_str());
    throw CMMError("Pixel size configuration preset " +
                       ToQuotedString(oldConfigName) + " does not exist",
                   MMERR_NoConfiguration);
  }
...
void CMMCore::deletePixelSizeConfig(const char *configName)
  ...
  if (!pixelSizeGroup_->Delete(configName)) {
    logError("deletePixelSizeConfig",
             getCoreErrorText(MMERR_NoConfiguration).c_str());
    throw CMMError("Pixel size configuration preset " +
                       ToQuotedString(configName) + " does not exist",
                   MMERR_NoConfiguration);
...
std::string CMMCore::getCurrentPixelSizeConfig(bool cached)
  ...
  for (size_t i = 0; i < cfgs.size(); i++) {
    ...
    for (size_t j = 0; j < cfgData->size(); j++) {
      ...
      if (!curState.isPropertyIncluded(cs.getDeviceLabel().c_str(),
                                       cs.getPropertyName().c_str())) {
        try {
          std::string value;
          if (!cached) {
            value = getProperty(cs.getDeviceLabel().c_str(),
                                cs.getPropertyName().c_str());
          } else {
            MMThreadGuard scg(stateCacheLock_);
            value = stateCache_
                        .getSetting(cs.getDeviceLabel().c_str(),
                                    cs.getPropertyName().c_str())
                        .getPropertyValue();
          }
          PropertySetting ss(cs.getDeviceLabel().c_str(),
 ...
 * This method is based on sensing the current pixel size configuration and
 * adjusting for the binning.
 */
double CMMCore::getPixelSizeUm() { return getPixelSizeUm(false); }
...
/**
 * Returns the current Affine Transform to related camera pixels with stage
 * movement.. This function returns the stored affine transform corrected for
 * binning
 */
 ...
 * Returns the current Affine Transform to related camera pixels with stage
 * movement.. This function returns the stored affine transform corrected for
 * binning and known magnification devices
 */
 ...
 *
 * @return products of all magnifier devices in the system or 1.0 when none is
 * found
 */
double CMMCore::getMagnificationFactor() const {
  double magnification = 1.0;
  std::vector<std::string> magnifiers =
      getLoadedDevicesOfType(MM::MagnifierDevice);
 ...
 * See: https://github.com/micro-manager/micro-manager/issues/1984
 *
 * @return        angle (dx/dz) of the Z-stage axis with the camera axis
 * (dimensionless)
 */
 ...
 * (dimensionless)
 */
double CMMCore::getPixelSizedxdz(const char *resolutionID)
    MMCORE_LEGACY_THROW(CMMError) {
  CheckConfigPresetName(resolutionID);
 ...
 * See: https://github.com/micro-manager/micro-manager/issues/1984
 *
 * @return   angle (dy/dz) of the Z-stage axis with the camera axis
 * (dimensionless)
 */
 ...
 * See: https://github.com/micro-manager/micro-manager/issues/1984
 *
 * @param cached   Uses System state cache to find active pixel size config when
 * true
 * @return   angle (dy/dz) of the Z-stage axis with the camera axis
 * (dimensionless)
 */
 ...
 * (dimensionless)
 */
double CMMCore::getPixelSizedydz(const char *resolutionID)
    MMCORE_LEGACY_THROW(CMMError) {
  CheckConfigPresetName(resolutionID);
...
/**
 * Returns the optimal z step size in um, optionally using cached pixel
 * configuration There is no magic to this number, but lets the system
 * configuration communicate to the end user what the optimal Z step size is for
 * this pixel size configuration
 *
 * @param cached   Uses System state cache to find active pixel size config when
 * true
 */
double CMMCore::getPixelSizeOptimalZUm(bool cached)
    MMCORE_LEGACY_THROW(CMMError) {
  std::string resolutionID;
  resolutionID = getCurrentPixelSizeConfig(cached);
 ...
 * this pixel size configuration
 */
double CMMCore::getPixelSizeOptimalZUm(const char *resolutionID)
    MMCORE_LEGACY_THROW(CMMError) {
  CheckConfigPresetName(resolutionID);
 ...
 */
void CMMCore::setSerialProperties(
    const char *portName, const char *answerTimeout, const char *baudRate,
    const char *delayBetweenCharsMs, const char *handshaking,
    const char *parity, const char *stopBits) MMCORE_LEGACY_THROW(CMMError) {
 ...
 * This command blocks until it receives an answer from the device terminated by
 * the specified sequence.
 */
void CMMCore::setSerialPortCommand(const char *portLabel, const char *command,
                                   const char *term)
    MMCORE_LEGACY_THROW(CMMError) {
...
void CMMCore::writeToSerialPort(const char *portLabel,
  ...
  int ret =
      pSerial->Write((unsigned char *)(&(data[0])), (unsigned long)data.size());
  if (ret != DEVICE_OK) {
    logError(portLabel, getDeviceErrorText(ret, pSerial).c_str());
    throw CMMError(getDeviceErrorText(ret, pSerial));
  }
 ...
 */
void CMMCore::setSLMImage(const char *deviceLabel, unsigned char *pixels)
    MMCORE_LEGACY_THROW(CMMError) {
 ...
 */
void CMMCore::setSLMImage(const char *deviceLabel, imgRGB32 pixels)
    MMCORE_LEGACY_THROW(CMMError) {
  std::shared_ptr<SLMInstance> pSLM =
      deviceManager_->GetDeviceOfType<SLMInstance>(deviceLabel);
  if (!pixels)
 ...
 */
void CMMCore::setSLMPixelsTo(const char *deviceLabel, unsigned char intensity)
    MMCORE_LEGACY_THROW(CMMError) {
  std::shared_ptr<SLMInstance> pSLM =
      deviceManager_->GetDeviceOfType<SLMInstance>(deviceLabel);
 ...
 */
void CMMCore::setSLMPixelsTo(const char *deviceLabel, unsigned char red,
 ...
 */
void CMMCore::setSLMExposure(const char *deviceLabel, double exposure_ms)
    MMCORE_LEGACY_THROW(CMMError) {
  std::shared_ptr<SLMInstance> pSLM =
      deviceManager_->GetDeviceOfType<SLMInstance>(deviceLabel);
...
/**
 * Set the Galvo to an x,y position and fire the laser for a predetermined
 * duration.
 */
void CMMCore::pointGalvoAndFire(const char *deviceLabel, double x, double y,
                                double pulseTime_us)
    MMCORE_LEGACY_THROW(CMMError) {
...
void CMMCore::setGalvoSpotInterval(const char *deviceLabel, double pulseTime_us)
    MMCORE_LEGACY_THROW(CMMError) {
  std::shared_ptr<GalvoInstance> pGalvo =
      deviceManager_->GetDeviceOfType<GalvoInstance>(deviceLabel);
 ...
 */
void CMMCore::setGalvoPosition(const char *deviceLabel, double x, double y)
    MMCORE_LEGACY_THROW(CMMError) {
  std::shared_ptr<GalvoInstance> pGalvo =
      deviceManager_->GetDeviceOfType<GalvoInstance>(deviceLabel);
 ...
 */
void CMMCore::getGalvoPosition(const char *deviceLabel, double &x, double &y)
    MMCORE_LEGACY_THROW(CMMError) {
  std::shared_ptr<GalvoInstance> pGalvo =
      deviceManager_->GetDeviceOfType<GalvoInstance>(deviceLabel);
 ...
 */
void CMMCore::setGalvoIlluminationState(const char *deviceLabel, bool on)
    MMCORE_LEGACY_THROW(CMMError) {
  std::shared_ptr<GalvoInstance> pGalvo =
      deviceManager_->GetDeviceOfType<GalvoInstance>(deviceLabel);
 ...
 */
void CMMCore::addGalvoPolygonVertex(const char *deviceLabel, int polygonIndex,
                                    double x, double y)
    MMCORE_LEGACY_THROW(CMMError) {
  std::shared_ptr<GalvoInstance> pGalvo =
      deviceManager_->GetDeviceOfType<GalvoInstance>(deviceLabel);
 ...
 */
void CMMCore::pressurePumpStop(const char *deviceLabel)
    MMCORE_LEGACY_THROW(CMMError) {
 ...
 * Calibrates the pump
 */
void CMMCore::pressurePumpCalibrate(const char *deviceLabel)
    MMCORE_LEGACY_THROW(CMMError) {
  std::shared_ptr<PressurePumpInstance> pPump =
      deviceManager_->GetDeviceOfType<PressurePumpInstance>(deviceLabel);
  mm::DeviceModuleLockGuard guard(pPump);
 ...
 */
bool CMMCore::pressurePumpRequiresCalibration(const char *deviceLabel)
    MMCORE_LEGACY_THROW(CMMError) {
  std::shared_ptr<PressurePumpInstance> pPump =
      deviceManager_->GetDeviceOfType<PressurePumpInstance>(deviceLabel);
  mm::DeviceModuleLockGuard guard(pPump);
 ...
 */
double CMMCore::getPumpPressureKPa(const char *deviceLabel)
    MMCORE_LEGACY_THROW(CMMError) {
  std::shared_ptr<PressurePumpInstance> pPump =
      deviceManager_->GetDeviceOfType<PressurePumpInstance>(deviceLabel);
  mm::DeviceModuleLockGuard guard(pPump);
 ...
 */
void CMMCore::setPumpPressureKPa(const char *deviceLabel, double pressurekPa)
    MMCORE_LEGACY_THROW(CMMError) {
  std::shared_ptr<PressurePumpInstance> pPump =
      deviceManager_->GetDeviceOfType<PressurePumpInstance>(deviceLabel);
  mm::DeviceModuleLockGuard guard(pPump);
 ...
 */
void CMMCore::volumetricPumpStop(const char *deviceLabel)
    MMCORE_LEGACY_THROW(CMMError) {
 ...
 * Homes the pump
 */
void CMMCore::volumetricPumpHome(const char *deviceLabel)
    MMCORE_LEGACY_THROW(CMMError) {
  std::shared_ptr<VolumetricPumpInstance> pPump =
      deviceManager_->GetDeviceOfType<VolumetricPumpInstance>(deviceLabel);
  mm::DeviceModuleLockGuard guard(pPump);
...
bool CMMCore::volumetricPumpRequiresHoming(const char *deviceLabel)
    MMCORE_LEGACY_THROW(CMMError) {
  std::shared_ptr<VolumetricPumpInstance> pPump =
      deviceManager_->GetDeviceOfType<VolumetricPumpInstance>(deviceLabel);
  mm::DeviceModuleLockGuard guard(pPump);
 ...
 */
void CMMCore::invertPumpDirection(const char *deviceLabel, bool invert)
    MMCORE_LEGACY_THROW(CMMError) {
  std::shared_ptr<VolumetricPumpInstance> pPump =
      deviceManager_->GetDeviceOfType<VolumetricPumpInstance>(deviceLabel);
  mm::DeviceModuleLockGuard guard(pPump);
 ...
 */
bool CMMCore::isPumpDirectionInverted(const char *deviceLabel)
    MMCORE_LEGACY_THROW(CMMError) {
  std::shared_ptr<VolumetricPumpInstance> pPump =
      deviceManager_->GetDeviceOfType<VolumetricPumpInstance>(deviceLabel);
  mm::DeviceModuleLockGuard guard(pPump);
 ...
 */
void CMMCore::setPumpVolume(const char *deviceLabel, double volUl)
    MMCORE_LEGACY_THROW(CMMError) {
  std::shared_ptr<VolumetricPumpInstance> pPump =
      deviceManager_->GetDeviceOfType<VolumetricPumpInstance>(deviceLabel);
  mm::DeviceModuleLockGuard guard(pPump);
 ...
 */
double CMMCore::getPumpVolume(const char *deviceLabel)
    MMCORE_LEGACY_THROW(CMMError) {
  std::shared_ptr<VolumetricPumpInstance> pPump =
      deviceManager_->GetDeviceOfType<VolumetricPumpInstance>(deviceLabel);
  mm::DeviceModuleLockGuard guard(pPump);
 ...
 */
void CMMCore::setPumpMaxVolume(const char *deviceLabel, double volUl)
    MMCORE_LEGACY_THROW(CMMError) {
  std::shared_ptr<VolumetricPumpInstance> pPump =
      deviceManager_->GetDeviceOfType<VolumetricPumpInstance>(deviceLabel);
  mm::DeviceModuleLockGuard guard(pPump);
 ...
 */
double CMMCore::getPumpMaxVolume(const char *deviceLabel)
    MMCORE_LEGACY_THROW(CMMError) {
  std::shared_ptr<VolumetricPumpInstance> pPump =
      deviceManager_->GetDeviceOfType<VolumetricPumpInstance>(deviceLabel);
  mm::DeviceModuleLockGuard guard(pPump);
 ...
 */
void CMMCore::setPumpFlowrate(const char *deviceLabel, double UlperSec)
    MMCORE_LEGACY_THROW(CMMError) {
  std::shared_ptr<VolumetricPumpInstance> pPump =
      deviceManager_->GetDeviceOfType<VolumetricPumpInstance>(deviceLabel);
  mm::DeviceModuleLockGuard guard(pPump);
 ...
 */
double CMMCore::getPumpFlowrate(const char *deviceLabel)
    MMCORE_LEGACY_THROW(CMMError) {
  std::shared_ptr<VolumetricPumpInstance> pPump =
      deviceManager_->GetDeviceOfType<VolumetricPumpInstance>(deviceLabel);
  mm::DeviceModuleLockGuard guard(pPump);
 ...
 */
void CMMCore::pumpDispenseDurationSeconds(const char *deviceLabel,
                                          double seconds)
    MMCORE_LEGACY_THROW(CMMError) {
  std::shared_ptr<VolumetricPumpInstance> pPump =
      deviceManager_->GetDeviceOfType<VolumetricPumpInstance>(deviceLabel);
 ...
 */
void CMMCore::pumpDispenseVolumeUl(const char *deviceLabel, double microLiter)
    MMCORE_LEGACY_THROW(CMMError) {
  std::shared_ptr<VolumetricPumpInstance> pPump =
      deviceManager_->GetDeviceOfType<VolumetricPumpInstance>(deviceLabel);
  mm::DeviceModuleLockGuard guard(pPump);
 ...
 * The file format is directly readable by the complementary loadSystemState()
 * command.
 */
void CMMCore::saveSystemState(const char *fileName)
  ...
  if (!os.is_open()) {
    ...
    throw CMMError(ToQuotedString(fileName) + ": " +
                       getCoreErrorText(MMERR_FileOpenFailed),
  ...
  for (size_t i = 0; i < config.size(); i++) {
    ...
    if (!isPropertyReadOnly(s.getDeviceLabel().c_str(),
                            s.getPropertyName().c_str())) {
...
/**
 * Loads the system configuration from the text file conforming to the MM
 * specific format. The configuration contains a list of commands to build the
 * desired system state from read-write properties.
 *
...
void CMMCore::loadSystemState(const char *fileName)
  ...
  const int maxLineLength =
      4 * MM::MaxStrLength + 4; // accommodate up to 4 strings and delimiters
...
/**
 * Saves the current system configuration to a text file of the MM specific
 * format. The configuration file records only the information essential to the
 * hardware setup: devices, labels, pre-initialization properties, and
 * configurations. The file format is the same as for the system state.
 */
void CMMCore::saveSystemConfiguration(const char *fileName)
  ...
  for (it = devices.begin(); it != devices.end(); it++) {
    std::shared_ptr<DeviceInstance> pDev = deviceManager_->GetDevice(*it);
    mm::DeviceModuleLockGuard guard(pDev);
    os << MM::g_CFGCommand_Device << "," << *it << ","
       << pDev->GetAdapterModule()->GetName() << "," << pDev->GetName() << '\n';
  }
  ...
  for (size_t i = 0; i < config.size(); i++) {
    ...
    std::shared_ptr<DeviceInstance> pDevice =
        deviceManager_->GetDevice(s.getDeviceLabel());
    if (pDevice) {
      mm::DeviceModuleLockGuard guard(pDevice);
      bool isPreInit =
          pDevice->GetPropertyInitStatus(s.getPropertyName().c_str());
      if (isPreInit) {
        os << MM::g_CFGCommand_Property << ',' << s.getDeviceLabel() << ','
           << s.getPropertyName() << ',' << s.getPropertyValue() << '\n';
      }
  ...
  for (it = devices.begin(); it != devices.end(); it++) {
    ...
    if (!parentID.empty()) {
      os << MM::g_CFGCommand_ParentID << ',' << device->GetLabel() << ','
         << parentID << '\n';
    }
  }
  ...
  for (std::vector<std::string>::const_iterator stageIt = stageLabels.begin(),
    ...
    os << MM::g_CFGCommand_FocusDirection << ',' << *stageIt << ',' << direction
       << '\n';
  }
  ...
  std::vector<std::string> deviceLabels =
      deviceManager_->GetDeviceList(MM::StateDevice);
  for (size_t i = 0; i < deviceLabels.size(); i++) {
    ...
    for (unsigned long j = 0; j < numPos; j++) {
      ...
      if (!stateLabel.empty()) {
        os << MM::g_CFGCommand_Label << ',' << deviceLabels[i] << ',' << j
           << ',' << stateLabel << '\n';
      }
    }
  }
  ...
  for (size_t i = 0; i < pixelSizeGroups.size(); i++) {
    ...
    os << MM::g_CFGCommand_PixelSize_um << ',' << pixelSizeGroups[i].c_str()
       << ',' << getPixelSizeUmByID(pixelSizeGroups[i].c_str()) << '\n';
    std::vector<double> affines =
        getPixelSizeAffineByID(pixelSizeGroups[i].c_str());
    if (affines.size() == 6) {
      os << MM::g_CFGCommand_PixelSizeAffine << ','
         << pixelSizeGroups[i].c_str() << ',';
      for (int l = 0; l < 5; l++) {
        os << affines[l] << ',';
      }
      os << affines[5] << '\n';
    ...
    os << MM::g_CFGCommand_PixelSizeOptimalZUm << ','
       << pixelSizeGroups[i].c_str() << ','
  ...
  std::shared_ptr<CameraInstance> camera = currentCameraDevice_.lock();
  if (camera) {
    os << MM::g_CFGCommand_Property << ',' << MM::g_Keyword_CoreDevice << ','
       << MM::g_Keyword_CoreCamera << ',' << camera->GetLabel() << '\n';
  }
  std::shared_ptr<ShutterInstance> shutter = currentShutterDevice_.lock();
  if (shutter) {
    os << MM::g_CFGCommand_Property << ',' << MM::g_Keyword_CoreDevice << ','
       << MM::g_Keyword_CoreShutter << ',' << shutter->GetLabel() << '\n';
  }
  std::shared_ptr<StageInstance> focus = currentFocusDevice_.lock();
  if (focus) {
    os << MM::g_CFGCommand_Property << ',' << MM::g_Keyword_CoreDevice << ','
       << MM::g_Keyword_CoreFocus << ',' << focus->GetLabel() << '\n';
  }
}
 ...
 * Loads the system configuration from the text file conforming to the MM
 * specific format. The configuration contains a list of commands to build the
 * desired system state: devices, labels, properties, and configurations.
 *
 * Format specification:
 * Each line consists of a number of string fields separated by "," (comma)
 * characters. Lines beginning with "#" are ignored (can be used for comments).
 * Each line in the file will be parsed by the system and as a result a
 * corresponding command will be immediately executed. The first field in the
 * line always specifies the command from the following set of values: Device -
 * executes loadDevice() Label - executes defineStateLabel() command Property -
 * executes setPropertyCommand() Configuration - ignored for backward
 * compatibility
 *
 * The remaining fields in the line will be used for corresponding command
 * parameters. The number of parameters depends on the actual command used.
 *
 * This function is not thread-safe.
 */
  ...
  try {
    ...
    try {
      ...
      LOG_ERROR(coreLogger_)
          << "Error occurred while unloading all devices: " << err.getFullMsg();
    }
...
void CMMCore::loadSystemConfigurationImpl(const char *fileName)
    MMCORE_LEGACY_THROW(CMMError) {
  if (!fileName)
    throw CMMError("Null filename");

  LOG_INFO(coreLogger_) << "Loading system configuration from:"
                        << ToQuotedString(fileName);
      ...
      try {
        ...
        if (tokens[0].compare(MM::g_CFGCommand_Device) == 0) {
          ...
          if (tokens.size() == 4)
            ...
            // ...assuming here that the last missing toke represents an empty
            // string
            setProperty(tokens[1].c_str(), tokens[2].c_str(), "");
          else
            throw CMMError(getCoreErrorText(MMERR_InvalidCFGEntry) + " (" +
          ...
          if (tokens.size() != 4)
            throw CMMError(getCoreErrorText(MMERR_InvalidCFGEntry) + " (" +
                               ToQuotedString(line) + ")",
                           MMERR_InvalidCFGEntry);
          defineStateLabel(tokens[1].c_str(), atol(tokens[2].c_str()),
                           tokens[3].c_str());
          ...
          // ------------------------------------
          if (tokens.size() == 6)
            defineConfig(tokens[1].c_str(), tokens[2].c_str(),
                         tokens[3].c_str(), tokens[4].c_str(),
                         tokens[5].c_str());
          else if (tokens.size() == 5) {
            // we will assume here that the last (missing) token is representing
            // an empty string
            defineConfig(tokens[1].c_str(), tokens[2].c_str(),
                         tokens[3].c_str(), tokens[4].c_str(), "");
          ...
          // ---------------------------------------
          if (tokens.size() == 5)
            definePixelSizeConfig(tokens[1].c_str(), tokens[2].c_str(),
                                  tokens[3].c_str(), tokens[4].c_str());
          else
            throw CMMError(getCoreErrorText(MMERR_InvalidCFGEntry) + " (" +
                               ToQuotedString(line) + ")",
          ...
          if (tokens.size() == 3)
            ...
            throw CMMError(getCoreErrorText(MMERR_InvalidCFGEntry) + " (" +
                               ToQuotedString(line) + ")",
                           MMERR_InvalidCFGEntry);
        } else if (tokens[0].compare(MM::g_CFGCommand_PixelSizeOptimalZUm) ==
                   0) {
          if (tokens.size() == 3)
 ...
 * This function is not thread safe.
 */
void CMMCore::registerCallback(MMEventCallback *cb) { externalCallback_ = cb; }
 ...
 * Use this value to create profiles or just to verify that the image is in
 * focus. The absolute range of returned scores depends on the actual focusing
 * device.
 */
...
void CMMCore::enableContinuousFocus(bool enable) MMCORE_LEGACY_THROW(CMMError) {
  ...
  if (autofocus) {
    ...
    if (ret != DEVICE_OK) {
      logError(getDeviceName(autofocus).c_str(),
               getDeviceErrorText(ret, autofocus).c_str());
      throw CMMError(getDeviceErrorText(ret, autofocus).c_str(),
                     MMERR_DEVICE_GENERIC);
    }
    ...
    if (enable) {
      ...
      throw CMMError(getCoreErrorText(MMERR_ContFocusNotAvailable).c_str(),
                     MMERR_ContFocusNotAvailable);
    }
  }
}
...
/**
 * Check if a stage has continuous focusing capability (positions can be set
 * while continuous focus runs).
 */
...
void CMMCore::fullFocus() MMCORE_LEGACY_THROW(CMMError) {
  ...
  if (autofocus) {
    ...
    throw CMMError(getCoreErrorText(MMERR_AutoFocusNotAvailable).c_str(),
                   MMERR_AutoFocusNotAvailable);
  }
}
...
void CMMCore::InitializeErrorMessages() {
  ...
  errorText_[MMERR_SetPropertyFailed] =
      "Property does not exist, or value not allowed.";
  errorText_[MMERR_LoadLibraryFailed] =
      "Unable to load library: file not accessible or corrupted.";
  errorText_[MMERR_LibraryFunctionNotFound] =
      "Unable to identify expected interface: the library is not compatible or "
      "corrupted.";
  errorText_[MMERR_CreateNotFound] =
      "Unable to identify CreateDevice function: the library is not compatible "
      "or corrupted.";
  errorText_[MMERR_DeleteNotFound] =
      "Unable to identify DeleteDevice function: the library is not compatible "
      "or corrupted.";
  errorText_[MMERR_CreateFailed] = "DeviceCreate function failed.";
  errorText_[MMERR_DeleteFailed] = "DeviceDelete function failed.";
  errorText_[MMERR_UnknownModule] =
      "Current device can't be unloaded: using unknown library.";
  ...
  errorText_[MMERR_DeviceVersionMismatch] =
      "Device interface version mismatch.";
  ...
  errorText_[MMERR_InvalidStateDevice] =
      "Unsupported API. This device is not a state device";
  errorText_[MMERR_NoConfiguration] = "Configuration not defined";
  errorText_[MMERR_InvalidPropertyBlock] =
      "Property block not defined"; // No longer used
  ...
  errorText_[MMERR_InvalidShutterDevice] =
      "Unsupported interface. This device is not a shutter.";
  ...
  errorText_[MMERR_InvalidCFGEntry] =
      "Invalid configuration file line encountered. Wrong number of tokens for "
      "the current context.";
  ...
  errorText_[MMERR_CircularBufferFailedToInitialize] =
      "Failed to initialize circular buffer - memory requirements not "
      "adequate.";
  errorText_[MMERR_CircularBufferEmpty] = "Circular buffer is empty.";
  errorText_[MMERR_ContFocusNotAvailable] =
      "Auto-focus focus device not defined.";
  errorText_[MMERR_BadConfigName] =
      "Configuration name contains illegal characters (/\\*!')";
  errorText_[MMERR_NotAllowedDuringSequenceAcquisition] =
      "This operation can not be executed while sequence acquisition is "
      "running.";
  ...
  errorText_[MMERR_CreatePeripheralFailed] =
      "Hub failed to create specified peripheral device.";
  errorText_[MMERR_BadAffineTransform] =
      "Bad affine transform.  Affine transforms need to have 6 numbers; 2 rows "
      "of 3 column.";
}
...
void CMMCore::CheckDeviceLabel(const char *label)
    MMCORE_LEGACY_THROW(CMMError) {
  if (!label)
  ...
  if (ContainsForbiddenCharacters(label))
    throw CMMError("Device label " + ToQuotedString(label) +
                       " contains reserved characters",
                   MMERR_InvalidContents);
}

void CMMCore::CheckPropertyName(const char *propName)
    MMCORE_LEGACY_THROW(CMMError) {
  if (!propName)
    throw CMMError("Null property name", MMERR_NullPointerException);
  if (ContainsForbiddenCharacters(propName))
    throw CMMError("Property name " + ToQuotedString(propName) +
                       " contains reserved characters",
                   MMERR_InvalidContents);
}

void CMMCore::CheckPropertyValue(const char *value)
    MMCORE_LEGACY_THROW(CMMError) {
  if (!value)
    throw CMMError("Null property value", MMERR_NullPointerException);
  if (ContainsForbiddenCharacters(value))
    throw CMMError("Property value " + ToQuotedString(value) +
                       " contains reserved characters",
                   MMERR_InvalidContents);
}

void CMMCore::CheckStateLabel(const char *stateLabel)
    MMCORE_LEGACY_THROW(CMMError) {
  if (!stateLabel)
    throw CMMError("Null state label", MMERR_NullPointerException);
  if (ContainsForbiddenCharacters(stateLabel))
    throw CMMError("State label " + ToQuotedString(stateLabel) +
                       " contains reserved characters",
                   MMERR_InvalidContents);
}

void CMMCore::CheckConfigGroupName(const char *groupName)
    MMCORE_LEGACY_THROW(CMMError) {
  if (!groupName)
    throw CMMError("Null configuration group name", MMERR_NullPointerException);
  if (ContainsForbiddenCharacters(groupName))
    throw CMMError("Configuration group name " + ToQuotedString(groupName) +
                       " contains reserved characters",
                   MMERR_InvalidContents);
}

void CMMCore::CheckConfigPresetName(const char *presetName)
    MMCORE_LEGACY_THROW(CMMError) {
  if (!presetName)
    throw CMMError("Null configuration preset name",
                   MMERR_NullPointerException);
...
bool CMMCore::IsCoreDeviceLabel(const char *label) const
    MMCORE_LEGACY_THROW(CMMError) {
  if (!label)
    throw CMMError("Null device label", MMERR_NullPointerException);
 ...
 */
void CMMCore::applyConfiguration(const Configuration &config)
    MMCORE_LEGACY_THROW(CMMError) {
  ...
  for (size_t i = 0; i < config.size(); i++) {
    ...
    if (setting.getDeviceLabel().compare(MM::g_Keyword_CoreDevice) == 0) {
      properties_->Execute(setting.getPropertyName().c_str(),
                           setting.getPropertyValue().c_str());
      {
        MMThreadGuard scg(stateCacheLock_);
        stateCache_.addSetting(PropertySetting(
            MM::g_Keyword_CoreDevice, setting.getPropertyName().c_str(),
            setting.getPropertyValue().c_str()));
      }
    } else {
      // normal processing
  ...
  if (error) {
    ...
    while (failedProps.size() >
           (unsigned)applyProperties(failedProps, errorString)) {
 ...
 * It is possible that setting certain properties failed because they are
 * dependent on other properties to be set first. As a workaround, continue to
 * apply these failed properties until there are none left or none succeed
 * returns number of properties successfully set
 */
int CMMCore::applyProperties(std::vector<PropertySetting> &props,
...
std::string
CMMCore::getDeviceErrorText(int deviceCode,
...
void CMMCore::logError(const char *device, const char *msg) {
  ...
  LOG_ERROR(coreLogger_) << "Error occurred in device " << device << ": "
                         << msg;
}
...
void CMMCore::updateAllowedChannelGroups() {
  ...
  // If we don't have the group assigned to ChannelGroup anymore, set
  // ChannelGroup to blank.
 ...
 *
 * @param label  the label of the device for which the serial port should be
 * found
 */
MM::DeviceDetectionStatus CMMCore::detectDevice(const char *label) {
  ...
  try {
    ...
    if (!port.empty()) {
      ...
      for (std::vector<std::string>::iterator sit = propertiesToRestore.begin();
           sit != propertiesToRestore.end(); ++sit) {
        try {
    ...
    LOG_ERROR(coreLogger_) << "Device detection: error testing ports "
                           << (port.empty() ? "none" : port) << " for device "
                           << label;
  }
  ...
  if (MM::CanCommunicate != result) {
    for (std::vector<std::string>::iterator sit = propertiesToRestore.begin();
      ...
      if (!port.empty()) {
        try {
          setProperty(port.c_str(), (*sit).c_str(),
                      (valuesToRestore[*sit]).c_str());
        } catch (...) {
          LOG_ERROR(coreLogger_)
 ...
 * Hub device. For example, if a motorized microscope is represented by a Hub
 * device, it is capable of discovering what specific child devices are
 * currently attached. In that case this call might report that Z-stage, filter
 * changer and objective turret are currently installed and return three device
 * names in the string list.
 *
 ...
 */
std::vector<std::string>
CMMCore::getInstalledDevices(const char *hubDeviceLabel)
    MMCORE_LEGACY_THROW(CMMError) {
...
std::vector<std::string>
CMMCore::getLoadedPeripheralDevices(const char *hubLabel)
    MMCORE_LEGACY_THROW(CMMError) {
  CheckDeviceLabel(hubLabel);
  return deviceManager_->GetLoadedPeripherals(hubLabel);
}

std::string CMMCore::getInstalledDeviceDescription(const char *hubLabel,
                                                   const char *deviceLabel)
    MMCORE_LEGACY_THROW(CMMError) {
  std::shared_ptr<HubInstance> pHub =
...
void CMMCore::loadMockDeviceAdapter(const char *name,
                                    MockDeviceAdapter *implementation)
    MMCORE_LEGACY_THROW(CMMError) {
  if (!name)
    throw CMMError("Null device adapter name");
